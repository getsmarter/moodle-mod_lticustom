...
require_once($CFG->dirroot.'/mod/lticustom/OAuthBody.php');
...
define('LTI_ITEM_MODULE', 'lticustom');
define('LTI_SOURCE', 'mod/lticustom');

function lticuston_get_response_xml($codemajor, $description, $messageref, $messagetype) {
...
function lticustom_parse_message_id($xml) {
...
function lticustom_parse_grade_replace_message($xml) {
    ...
    $parsed->messageid = lticustom_parse_message_id($xml);

    return $parsed;
}

function lticustom_parse_grade_read_message($xml) {
...
function lticustom_parse_grade_delete_message($xml) {
...
function lticustom_accepts_grades($ltiinstance) {
    global $DB;

    $acceptsgrades = true;
    $ltitype = $DB->get_record('lticustom_types', array('id' => $ltiinstance->typeid));

    if (empty($ltitype->toolproxyid)) {
        $typeconfig = lticustom_get_config($ltiinstance);

        $typeacceptgrades = isset($typeconfig['acceptgrades']) ? $typeconfig['acceptgrades'] : LTI_CUSTOM_SETTING_DELEGATE;

        if (!($typeacceptgrades == LTI_CUSTOM_SETTING_ALWAYS ||
            ($typeacceptgrades == LTI_CUSTOM_SETTING_DELEGATE && $ltiinstance->instructorchoiceacceptgrades == LTI_CUSTOM_SETTING_ALWAYS))) {
            $acceptsgrades = false;
        }
    } else {
 ...
 * @param int $userid
 */
function lticustom_set_session_user($userid) {
    global $DB;
...
function lticustom_update_grade($ltiinstance, $userid, $launchid, $gradeval) {
    global $CFG, $DB;
    ...
    $record = $DB->get_record('lticustom_submission', array('ltiid' => $ltiinstance->id, 'userid' => $userid,
    ...
    if (!empty($id)) {
        $DB->update_record('lticustom_submission', array(
            'id' => $id,
            'dateupdated' => time(),
        ...
        $DB->insert_record('lticustom_submission', array(
...
function lticustom_read_grade($ltiinstance, $userid) {
    global $CFG;
    require_once($CFG->libdir . '/gradelib.php');
...
function lticustom_delete_grade($ltiinstance, $userid) {
    global $CFG;
    require_once($CFG->libdir . '/gradelib.php');
...
function lticustom_verify_message($key, $sharedsecrets, $body, $headers = null) {
 ...
 * @throws Exception
 */
function lticustom_verify_sourcedid($ltiinstance, $parsed) {
    $sourceid = lticustom_build_sourcedid($parsed->instanceid, $parsed->userid,
 ...
 */
function lticustom_extend_lti_services($data) {
    $plugins = get_plugin_list_with_function('lticustomsource', $data->messagetype);
    if (!empty($plugins)) {
