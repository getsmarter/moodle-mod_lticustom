 ...
 */
function lticustom_supports($feature) {
    switch ($feature) {
        case FEATURE_GROUPS:
 ...
 **/
function lticustom_add_instance($lti, $mform) {
    global $DB, $CFG;
    ...
    lticustom_force_type_config_settings($lti, lticustom_get_type_config_by_instance($lti));

    if (empty($lti->typeid) && isset($lti->urlmatchedtypeid)) {
        $lti->typeid = $lti->urlmatchedtypeid;
    }

    if (!isset($lti->instructorchoiceacceptgrades) || $lti->instructorchoiceacceptgrades != LTI_CUSTOM_SETTING_ALWAYS) {
        // The instance does not accept grades back from the provider, so set to "No grade" value 0.
        $lti->grade = 0;
    }

    $lti->id = $DB->insert_record('lticustom', $lti);
    ...
    if (isset($lti->instructorchoiceacceptgrades) && $lti->instructorchoiceacceptgrades == LTI_CUSTOM_SETTING_ALWAYS) {
        ...
        lticustom_grade_item_update($lti);
    }

    $completiontimeexpected = !empty($lti->completionexpected) ? $lti->completionexpected : null;
    \core_completion\api::update_completion_date_event($lti->coursemodule, 'lticustom', $lti->id, $completiontimeexpected);
 ...
 **/
function lticustom_update_instance($lti, $mform) {
    global $DB, $CFG;
    require_once($CFG->dirroot.'/mod/lticustom/locallib.php');
    ...
    if (isset($lti->instructorchoiceacceptgrades) && $lti->instructorchoiceacceptgrades == LTI_CUSTOM_SETTING_ALWAYS) {
        ...
        lticustom_grade_item_delete($lti);
    }
    ...
    return $DB->update_record('lticustom', $lti);
}
...
function lticustom_delete_instance($id) {
    ...
    lticustom_grade_item_delete($basiclti);

    $ltitype = $DB->get_record('lticustom_types', array('id' => $basiclti->typeid));
    if ($ltitype) {
        $DB->delete_records('lticustom_tool_settings',
            array('toolproxyid' => $ltitype->toolproxyid, 'course' => $basiclti->course, 'coursemoduleid' => $id));
    }

    $cm = get_coursemodule_from_instance('lticustom', $id);
 ...
 **/
function lticustom_get_shortcuts($defaultitem) {
    global $CFG, $COURSE;
    require_once($CFG->dirroot.'/mod/lticustom/locallib.php');

    $types = lticustom_get_configured_types($COURSE->id, $defaultitem->link->param('sr'));
    $types[] = $defaultitem;

    // Add items defined in ltisource plugins.
    foreach (core_component::get_plugin_list('lticustomsource') as $pluginname => $dir) {
        ...
        if ($moretypes = component_callback("lticustomsource_$pluginname", 'get_shortcuts', array($defaultitem))) {
 ...
 */
function lticustom_get_coursemodule_info($coursemodule) {
    global $DB, $CFG;
    require_once($CFG->dirroot.'/mod/lticustom/locallib.php');

    if (!$lti = $DB->get_record('lticustom', array('id' => $coursemodule->instance),
    ...
    if ($coursemodule->showdescription) {
        ...
        $info->content = format_module_intro('lticustom', $lti, $coursemodule->id, false);
    }

    if (!empty($lti->typeid)) {
        $toolconfig = lticustom_get_type_config($lti->typeid);
    } else if ($tool = lticustom_get_tool_by_url_match($lti->toolurl)) {
        $toolconfig = lticustom_get_type_config($tool->id);
    } else {
        $toolconfig = array();
    }
    ...
    if (lticustom_request_is_using_ssl() &&
    ...
    $launchcontainer = lticustom_get_launch_container($lti, $toolconfig);
    if ($launchcontainer == LTI_CUSTOM_LAUNCH_CONTAINER_WINDOW) {
        $launchurl = new moodle_url('/mod/lticustom/launch.php', array('id' => $coursemodule->id));
 ...
 **/
function lticustom_user_outline($course, $user, $mod, $basiclti) {
    return null;
}
 ...
 * @TODO: implement this moodle function (if needed)
 **/
function lticustom_user_complete($course, $user, $mod, $basiclti) {
    return true;
}
 ...
 **/
function lticustom_print_recent_activity($course, $isteacher, $timestart) {
 ...
 * @TODO: implement this moodle function (if needed)
 **/
function lticustom_grades($basicltiid) {
    return null;
}
 ...
 * @TODO: implement this moodle function (if needed)
 **/
function lticustom_scale_used ($basicltiid, $scaleid) {
    $return = false;
 ...
 *
 */
function lticustom_scale_used_anywhere($scaleid) {
    global $DB;

    if ($scaleid and $DB->record_exists('lticustom', array('grade' => -$scaleid))) {
        return true;
    } else {
        return false;
    }
 ...
 */
function lticustom_get_lti_types() {
    global $DB;

    return $DB->get_records('lticustom_types', null, 'state DESC, timemodified DESC');
}
 ...
 * @return array of basicLTI types
 */
function lticustom_get_lti_types_from_proxy_id($toolproxyid) {
    global $DB;

    return $DB->get_records('lticustom_types', array('toolproxyid' => $toolproxyid), 'state DESC, timemodified DESC');
}
 ...
 */
function lticustom_grade_item_update($basiclti, $grades = null) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');
    require_once($CFG->dirroot.'/mod/lticustom/servicelib.php');

    if (!lticustom_accepts_grades($basiclti)) {
        return 0;
    }
    ...
    return grade_update('mod/lticustom', $basiclti->course, 'mod', 'lticustom', $basiclti->id, 0, $grades, $params);
}
 ...
 */
function lticustom_update_grades($basiclti, $userid=0, $nullifnone=true) {
    global $CFG;
    require_once($CFG->dirroot.'/mod/lticustom/servicelib.php');
    // LTI doesn't have its own grade table so the only thing to do is update the grade item.
    if (lticustom_accepts_grades($basiclti)) {
        lticustom_grade_item_update($basiclti);
    }
}
 ...
 */
function lticustom_grade_item_delete($basiclti) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');

    return grade_update('mod/lticustom', $basiclti->course, 'mod', 'lticustom', $basiclti->id, 0, null, array('deleted' => 1));
}
 ...
 * @return array
 */
function lticustom_get_post_actions() {
    return array();
}
 ...
 * @return array
 */
function lticustom_get_view_actions() {
 ...
 * @since Moodle 3.0
 */
function lticustom_view($lti, $course, $cm, $context) {
    ...
    $event = \mod_lticustom\event\course_module_viewed::create($params);
    $event->add_record_snapshot('course_modules', $cm);
    $event->add_record_snapshot('course', $course);
    $event->add_record_snapshot('lticustom', $lti);
    $event->trigger();
 ...
 * @since Moodle 3.2
 */
function lticustom_check_updates_since(cm_info $cm, $from, $filter = array()) {
    global $DB, $USER;
    ...
    $submissions = $DB->get_records_select('lticustom_submission', $select, $params, '', 'id');
    ...
    if (has_capability('mod/lticustom:manage', $cm->context)) {
 ...
 */
function mod_lticustom_get_fontawesome_icon_map() {
    return [
        'mod_lticustom:warning' => 'fa-exclamation text-warning',
    ];
}
 ...
 */
function mod_lticustom_core_calendar_provide_event_action(calendar_event $event,
    ...
    $cm = get_fast_modinfo($event->courseid, $userid)->instances['lticustom'][$event->instance];
    ...
    return $factory->create_instance(
        get_string('view'),
        new \moodle_url('/mod/lticustom/view.php', ['id' => $cm->id]),
        1,
        true
    );
}
