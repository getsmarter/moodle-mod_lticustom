 ...
 */
function mod_lticustom_verify_private_key() {
    $key = get_config('privatekey', 'mod_lticustom');
    ...
    if (empty($key)) {
        ...
        set_config('kid', $kid, 'mod_lticustom');
        $config = array(
            "digest_alg" => "sha256",
        ...
        if (!empty($privatekey)) {
            set_config('privatekey', $privatekey, 'mod_lticustom');
        } else {
            return get_string('opensslconfiginvalid', 'mod_lticustom');
        }
    }
