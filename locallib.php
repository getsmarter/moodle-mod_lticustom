...
global $CFG;
require_once($CFG->dirroot.'/mod/lticustom/OAuth.php');
require_once($CFG->libdir.'/weblib.php');
require_once($CFG->dirroot . '/course/modlib.php');
require_once($CFG->dirroot . '/mod/lticustom/TrivialStore.php');

define('LTI_CUSTOM_URL_DOMAIN_REGEX', '/(?:https?:\/\/)?(?:www\.)?([^\/]+)(?:\/|$)/i');

define('LTI_CUSTOM__LAUNCH_CONTAINER_DEFAULT', 1);
define('LTI_CUSTOM_LAUNCH_CONTAINER_EMBED', 2);
define('LTI_CUSTOM_LAUNCH_CONTAINER_EMBED_NO_BLOCKS', 3);
define('LTI_CUSTOM_LAUNCH_CONTAINER_WINDOW', 4);
define('LTI_CUSTOM_LAUNCH_CONTAINER_REPLACE_MOODLE_WINDOW', 5);

define('LTI_CUSTOM_TOOL_STATE_ANY', 0);
define('LTI_CUSTOM_TOOL_STATE_CONFIGURED', 1);
define('LTI_CUSTOM_TOOL_STATE_PENDING', 2);
define('LTI_CUSTOM_TOOL_STATE_REJECTED', 3);
define('LTI_CUSTOM_TOOL_PROXY_TAB', 4);

define('LTI_CUSTOM_TOOL_PROXY_STATE_CONFIGURED', 1);
define('LTI_CUSTOM_TOOL_PROXY_STATE_PENDING', 2);
define('LTI_CUSTOM_TOOL_PROXY_STATE_ACCEPTED', 3);
define('LTI_CUSTOM_TOOL_PROXY_STATE_REJECTED', 4);

define('LTI_CUSTOM_SETTING_NEVER', 0);
define('LTI_CUSTOM_SETTING_ALWAYS', 1);
define('LTI_CUSTOM_SETTING_DELEGATE', 2);

define('LTI_CUSTOM_COURSEVISIBLE_NO', 0);
define('LTI_CUSTOM_COURSEVISIBLE_PRECONFIGURED', 1);
define('LTI_CUSTOM_COURSEVISIBLE_ACTIVITYCHOOSER', 2);

define('LTI_CUSTOM_VERSION_1', 'LTI-1p0');
define('LTI_CUSTOM_VERSION_2', 'LTI-2p0');
define('LTI_CUSTOM_VERSION_1P3', '1.3.0');

define('LTI_CUSTOM_ACCESS_TOKEN_LIFE', 3600);

// Standard prefix for JWT claims.
define('LTI_CUSTOM_JWT_CLAIM_PREFIX', 'https://purl.imsglobal.org/spec/lti');
 ...
 * @return array
 */
function lticustom_get_jwt_message_type_mapping() {
    return array(
 ...
 * @return array
 */
function lticustom_get_jwt_claim_mapping() {
    return array(
        'accept_copy_advice' => [
            'suffix' => 'dl',
 ...
 * @since  Moodle 3.0
 */
function lticustom_get_launch_data($instance, $nonce = '') {
    global $PAGE, $CFG, $USER;

    if (empty($instance->typeid)) {
        $tool = lticustom_get_tool_by_url_match($instance->toolurl, $instance->course);
        if ($tool) {
            $typeid = $tool->id;
            $ltiversion = $tool->ltiversion;
        } else {
            $tool = lticustom_get_tool_by_url_match($instance->securetoolurl,  $instance->course);
            if ($tool) {
                $typeid = $tool->id;
                $ltiversion = $tool->ltiversion;
            } else {
                $typeid = null;
                $ltiversion = LTI_CUSTOM_VERSION_1;
            }
        }
    } else {
        $typeid = $instance->typeid;
        $tool = lticustom_get_type($typeid);
        $ltiversion = $tool->ltiversion;
    }

    if ($typeid) {
        $typeconfig = lticustom_get_type_config($typeid);
    } else {
    ...
    if (isset($tool->toolproxyid)) {
        $toolproxy = lticustom_get_tool_proxy($tool->toolproxyid);
        $key = $toolproxy->guid;
        ...
        } else if ($ltiversion === LTI_CUSTOM_VERSION_1P3) {
            $key = $tool->clientid;
    ...
    if (lticustom_request_is_using_ssl() && !empty($instance->securetoolurl)) {
    ...
    if (isset($typeconfig['forcessl']) && ($typeconfig['forcessl'] == '1')) {
        ...
        $endpoint = lticustom_ensure_url_is_https($endpoint);
    } else {
        if (!strstr($endpoint, '://')) {
    ...
    $allparams = lticustom_build_request($instance, $typeconfig, $course, $typeid, $islti2);
    if ($islti2) {
        $requestparams = lticustom_build_request_lti2($tool, $allparams);
    } else {
        $requestparams = $allparams;
    }
    $requestparams = array_merge($requestparams, lticustom_build_standard_message($instance, $orgid, $ltiversion));
    $customstr = '';
    if (isset($typeconfig['customparameters'])) {
        $customstr = $typeconfig['customparameters'];
    }
    $requestparams = array_merge($requestparams, lticustom_build_custom_parameters($toolproxy, $tool, $instance, $allparams, $customstr,
        $instance->instructorcustomparameters, $islti2));

    $launchcontainer = lticustom_get_launch_container($instance, $typeconfig);
    ...
    $url = new \moodle_url('/mod/lticustom/return.php', $returnurlparams);
    $returnurl = $url->out(false);

    if (isset($typeconfig['forcessl']) && ($typeconfig['forcessl'] == '1')) {
        $returnurl = lticustom_ensure_url_is_https($returnurl);
    }

    $target = '';
    switch($launchcontainer) {
        case LTI_CUSTOM_LAUNCH_CONTAINER_EMBED:
        case LTI_CUSTOM_LAUNCH_CONTAINER_EMBED_NO_BLOCKS:
            $target = 'iframe';
            break;
        case LTI_CUSTOM_LAUNCH_CONTAINER_REPLACE_MOODLE_WINDOW:
            $target = 'frame';
            break;
        case LTI_CUSTOM_LAUNCH_CONTAINER_WINDOW:
            $target = 'window';
            break;
    }
    if (!empty($target)) {
    ...
    if ($typeid && !$islti2) {
        ...
        foreach ($services as $service) {
            ...
            foreach ($serviceparameters as $paramkey => $paramvalue) {
                $requestparams['custom_' . $paramkey] = lticustom_parse_custom_parameter($toolproxy, $tool, $requestparams, $paramvalue,
                    $islti2);
            }
        }
    ...
    $plugins = core_component::get_plugin_list('lticustomsource');
    foreach (array_keys($plugins) as $plugin) {
        $pluginparams = component_callback('lticustomsource_'.$plugin, 'before_launch',
    ...
    if ((!empty($key) && !empty($secret)) || ($ltiversion === LTI_CUSTOM_VERSION_1P3)) {
        if ($ltiversion !== LTI_CUSTOM_VERSION_1P3) {
            $parms = lticustom_sign_parameters($requestparams, $endpoint, 'POST', $key, $secret);
        } else {
            $parms = lticustom_sign_jwt($requestparams, $endpoint, $key, $typeid, $nonce);
        }
 ...
 */
function lticustom_launch_tool($instance) {

    list($endpoint, $parms) = lticustom_get_launch_data($instance);
    $debuglaunch = ( $instance->debuglaunch == 1 );

    $content = lticustom_post_launch_html($parms, $endpoint, $debuglaunch);
 ...
 */
function lticustom_register($toolproxy) {
    $endpoint = $toolproxy->regurl;

    // Change the status to pending.
    $toolproxy->state = LTI_CUSTOM_TOOL_PROXY_STATE_PENDING;
    lticustom_update_tool_proxy($toolproxy);

    $requestparams = lticustom_build_registration_request($toolproxy);

    $content = lticustom_post_launch_html($requestparams, $endpoint, false);
 ...
 */
function lticustom_build_registration_request($toolproxy) {
    $key = $toolproxy->guid;
    $secret = $toolproxy->secret;
    ...
    // Add the profile URL.
    $profileservice = lticustom_get_service_by_name('profile');
    ...
    $url = new \moodle_url('/mod/lticustom/externalregistrationreturn.php', $returnurlparams);
    $returnurl = $url->out(false);
 ...
 * @return stdClass
 */
function lticustom_build_sourcedid($instanceid, $userid, $servicesalt, $typeid = null, $launchid = null) {
    $data = new \stdClass();
 ...
 */
function lticustom_build_request($instance, $typeconfig, $course, $typeid = null, $islti2 = false) {
    global $USER, $CFG;
    ...
    $role = lticustom_get_ims_role($USER, $instance->cmid, $instance->course, $islti2);
    ...
    if (!empty($instance->cmid)) {
        $intro = format_module_intro('lticustom', $instance, $instance->cmid);
    ...
    if (!empty($instance->id) && !empty($instance->servicesalt) && ($islti2 ||
            $typeconfig['acceptgrades'] == LTI_CUSTOM_SETTING_ALWAYS ||
            ($typeconfig['acceptgrades'] == LTI_CUSTOM_SETTING_DELEGATE && $instance->instructorchoiceacceptgrades == LTI_CUSTOM_SETTING_ALWAYS))
    ) {
        $placementsecret = $instance->servicesalt;
        $sourcedid = json_encode(lticustom_build_sourcedid($instance->id, $USER->id, $placementsecret, $typeid));
        $requestparams['lis_result_sourcedid'] = $sourcedid;

        // Add outcome service URL.
        $serviceurl = new \moodle_url('/mod/lticustom/service.php');
        ...
        if ((isset($typeconfig['forcessl']) && ($typeconfig['forcessl'] == '1')) or $forcessl) {
            $serviceurl = lticustom_ensure_url_is_https($serviceurl);
        }
    ...
    if ($islti2 || $typeconfig['sendname'] == LTI_CUSTOM_SETTING_ALWAYS ||
        ($typeconfig['sendname'] == LTI_CUSTOM_SETTING_DELEGATE && isset($instance->instructorchoicesendname)
            && $instance->instructorchoicesendname == LTI_CUSTOM_SETTING_ALWAYS)
    ) {
    ...
    if ($islti2 || $typeconfig['sendemailaddr'] == LTI_CUSTOM_SETTING_ALWAYS ||
        ($typeconfig['sendemailaddr'] == LTI_CUSTOM_SETTING_DELEGATE && isset($instance->instructorchoicesendemailaddr)
            && $instance->instructorchoicesendemailaddr == LTI_CUSTOM_SETTING_ALWAYS)
    ) {
 ...
 * @return array                    Request details
 */
function lticustom_build_request_lti2($tool, $params) {

    $requestparams = array();

    $capabilities = lticustom_get_capabilities();
 ...
 * @see lticustom_build_standard_message()
 */
function lticustom_build_standard_request($instance, $orgid, $islti2, $messagetype = 'basic-lti-launch-request') {
    if (!$islti2) {
        $ltiversion = LTI_CUSTOM_VERSION_1;
    } else {
        $ltiversion = LTI_CUSTOM_VERSION_2;
    }
    return lticustom_build_standard_message($instance, $orgid, $ltiversion, $messagetype);
}
 ...
 */
function lticustom_build_standard_message($instance, $orgid, $ltiversion, $messagetype = 'basic-lti-launch-request') {
    global $CFG;
    ...
    if (!empty($CFG->mod_lticustom_institution_name)) {
        $requestparams['tool_consumer_instance_name'] = trim(html_to_text($CFG->mod_lticustom_institution_name, 0));
    } else {
 ...
 */
function lticustom_build_custom_parameters($toolproxy, $tool, $instance, $params, $customstr, $instructorcustomstr, $islti2) {
    ...
    $custom = array();
    if ($customstr) {
        $custom = lticustom_split_custom_parameters($toolproxy, $tool, $params, $customstr, $islti2);
    }
    if ($instructorcustomstr) {
        $custom = array_merge(lticustom_split_custom_parameters($toolproxy, $tool, $params,
            $instructorcustomstr, $islti2), $custom);
    }
    if ($islti2) {
        $custom = array_merge(lticustom_split_custom_parameters($toolproxy, $tool, $params,
            $tool->parameter, true), $custom);
        $settings = lticustom_get_tool_settings($tool->toolproxyid);
        $custom = array_merge($custom, lticustom_get_custom_parameters($toolproxy, $tool, $params, $settings));
        if (!empty($instance->course)) {
            $settings = lticustom_get_tool_settings($tool->toolproxyid, $instance->course);
            $custom = array_merge($custom, lticustom_get_custom_parameters($toolproxy, $tool, $params, $settings));
            if (!empty($instance->id)) {
                $settings = lticustom_get_tool_settings($tool->toolproxyid, $instance->course, $instance->id);
                $custom = array_merge($custom, lticustom_get_custom_parameters($toolproxy, $tool, $params, $settings));
            }
        }
    }
 ...
 */
function lticustom_build_content_item_selection_request($id, $course, moodle_url $returnurl, $title = '', $text = '', $mediatypes = [],
                                                  $presentationtargets = [], $autocreate = false, $multiple = false,
                                                  $unsigned = false, $canconfirm = false, $copyadvice = false, $nonce = '') {
    global $USER;

    $tool = lticustom_get_type($id);
    // Validate parameters.
    if (!$tool) {
        throw new moodle_exception('errortooltypenotfound', 'mod_lticustom');
    }
    ...
    $typeconfig = lticustom_get_type_config($id);
    $key = '';
    $secret = '';
    $islti2 = false;
    $islti13 = false;
    if (isset($tool->toolproxyid)) {
        ...
        $islti13 = $tool->ltiversion === LTI_CUSTOM_VERSION_1P3;
        $toolproxy = null;
    ...
    $requestparams = lticustom_build_request($instance, $typeconfig, $course, $id, $islti2);

    // Get LTI2-specific request parameters and merge to the request parameters if applicable.
    if ($islti2) {
        $lti2params = lticustom_build_request_lti2($tool, $requestparams);
    ...
    $standardparams = lticustom_build_standard_message(null, $orgid, $tool->ltiversion, 'ContentItemSelectionRequest');
    ...
    $customparams = lticustom_build_custom_parameters($toolproxy, $tool, $instance, $requestparams, $customstr, '', $islti2);
    ...
    $plugins = core_component::get_plugin_list('lticustomsource');
    foreach (array_keys($plugins) as $plugin) {
        $pluginparams = component_callback('lticustomsource_' . $plugin, 'before_launch', [$instance, $toolurlout, $requestparams], []);
    ...
    if (!$islti13) {
        $signedparams = lticustom_sign_parameters($requestparams, $toolurlout, 'POST', $key, $secret);
    } else {
        $signedparams = lticustom_sign_jwt($requestparams, $toolurlout, $key, $id, $nonce);
    }
 ...
 */
function lticustom_verify_oauth_signature($typeid, $consumerkey) {
    $tool = lticustom_get_type($typeid);
    // Validate parameters.
    if (!$tool) {
        throw new moodle_exception('errortooltypenotfound', 'mod_lticustom');
    ...
    if ($consumerkey !== $key) {
        throw new moodle_exception('errorincorrectconsumerkey', 'mod_lticustom');
    }
 ...
 */
function lticustom_verify_jwt_signature($typeid, $consumerkey, $jwtparam) {
    $tool = lticustom_get_type($typeid);
    // Validate parameters.
    if (!$tool) {
        throw new moodle_exception('errortooltypenotfound', 'mod_lticustom');
 ...
 * @throws lti\OAuthException
 */
function lticustom_tool_configuration_from_content_item($typeid, $messagetype, $ltiversion, $consumerkey, $contentitemsjson) {
    $tool = lticustom_get_type($typeid);
    // Validate parameters.
    if (!$tool) {
        throw new moodle_exception('errortooltypenotfound', 'mod_lticustom');
    ...
    $islti2 = ($expectedversion === LTI_CUSTOM_VERSION_2);
    ...
    if (empty($items)) {
        throw new moodle_exception('errorinvaliddata', 'mod_lticustom', '', $contentitemsjson);
    }
    if (!isset($items->{'@graph'}) || !is_array($items->{'@graph'}) || (count($items->{'@graph'}) > 1)) {
        throw new moodle_exception('errorinvalidresponseformat', 'mod_lticustom');
    }

    $config = null;
    if (!empty($items->{'@graph'})) {
        $item = $items->{'@graph'}[0];
        $typeconfig = lticustom_get_type_type_config($tool->id);
        ...
        $config->instructorchoiceacceptgrades = LTI_CUSTOM_SETTING_NEVER;
        if (!$islti2 && isset($typeconfig->lti_acceptgrades)) {
            $acceptgrades = $typeconfig->lti_acceptgrades;
            if ($acceptgrades == LTI_CUSTOM_SETTING_ALWAYS) {
                // We create a line item regardless if the definition contains one or not.
                $config->instructorchoiceacceptgrades = LTI_CUSTOM_SETTING_ALWAYS;
            }
            if ($acceptgrades == LTI_CUSTOM_SETTING_DELEGATE || $acceptgrades == LTI_CUSTOM_SETTING_ALWAYS) {
        ...
        $config->instructorchoicesendname = LTI_CUSTOM_SETTING_NEVER;
        $config->instructorchoicesendemailaddr = LTI_CUSTOM_SETTING_NEVER;
        $config->launchcontainer = LTI_CUSTOM__LAUNCH_CONTAINER_DEFAULT;
        if (isset($item->placementAdvice->presentationDocumentTarget)) {
            if ($item->placementAdvice->presentationDocumentTarget === 'window') {
                $config->launchcontainer = LTI_CUSTOM_LAUNCH_CONTAINER_WINDOW;
            } else if ($item->placementAdvice->presentationDocumentTarget === 'frame') {
                $config->launchcontainer = LTI_CUSTOM_LAUNCH_CONTAINER_EMBED_NO_BLOCKS;
            } else if ($item->placementAdvice->presentationDocumentTarget === 'iframe') {
                $config->launchcontainer = LTI_CUSTOM_LAUNCH_CONTAINER_EMBED;
            }
        }
 ...
 */
function lticustom_convert_content_items($param) {
    $items = array();
...
function lticustom_get_tool_table($tools, $id) {
    global $OUTPUT;
    $html = '';

    $typename = get_string('typename','lticustom');
    $baseurl = get_string('baseurl','lticustom');
    ...
    if (!empty($tools)) {
        ...
        foreach ($tools as $type) {
            ...
            $accept = get_string('accept','lticustom');
            $update = get_string('update','lticustom');
            $delete = get_string('delete','lticustom');

            if (empty($type->toolproxyid)) {
                $baseurl = new \moodle_url('/mod/lticustom/typessettings.php', array(
                        'action' => 'accept',
                        'id' => $type->id,
                        'sesskey' => sesskey(),
                        'tab' => $id
                ...
                $baseurl = new \moodle_url('/mod/lticustom/toolssettings.php', array(
                        'action' => 'accept',
                        'id' => $type->id,
                        'sesskey' => sesskey(),
                        'tab' => $id
            ...
            if ($type->state == LTI_CUSTOM_TOOL_STATE_CONFIGURED) {
                $accepthtml = '';
            }

            if ($type->state != LTI_CUSTOM_TOOL_STATE_REJECTED) {
                $deleteaction = 'reject';
                $delete = get_string('reject','lticustom');
            }
            ...
            if (($type->state != LTI_CUSTOM_TOOL_STATE_REJECTED) || empty($type->toolproxyid)) {
        ...
        $html .= get_string('no_' . $id, 'lticustom');
    }
 ...
 */
function lticustom_get_tool_proxy_table($toolproxies, $id) {
    global $OUTPUT;
...
EOD;
        foreach ($toolproxies as $toolproxy) {
            ...
            $accept = get_string('register','lticustom');
            $update = get_string('update','lticustom');
            $delete = get_string('delete','lticustom');

            $baseurl = new \moodle_url('/mod/lticustom/registersettings.php', array(
                    'action' => 'accept',
                    'id' => $toolproxy->id,
                    'sesskey' => sesskey(),
                    'tab' => $id
            ...
            $registerurl = new \moodle_url('/mod/lticustom/register.php', array(
                    'id' => $toolproxy->id,
                    'sesskey' => sesskey(),
                    'tab' => 'tool_proxy'
                ));
            ...
            if ($toolproxy->state != LTI_CUSTOM_TOOL_PROXY_STATE_CONFIGURED) {
                $accepthtml = '';
            }

            if (($toolproxy->state == LTI_CUSTOM_TOOL_PROXY_STATE_CONFIGURED) || ($toolproxy->state == LTI_CUSTOM_TOOL_PROXY_STATE_PENDING)) {
                $delete = get_string('cancel','lticustom');
            }
 ...
 */
function lticustom_get_enabled_capabilities($tool) {
    if (!isset($tool)) {
        return array();
    }
 ...
 */
function lticustom_split_custom_parameters($toolproxy, $tool, $params, $customstr, $islti2 = false) {
    ...
    foreach ($lines as $line) {
        ...
        $val = lticustom_parse_custom_parameter($toolproxy, $tool, $params, $val, $islti2);
        $key2 = lticustom_map_keyname($key);
        $retval['custom_'.$key2] = $val;
        if (($islti2 || ($tool->ltiversion === LTI_CUSTOM_VERSION_1P3)) && ($key != $key2)) {
 ...
 */
function lticustom_get_custom_parameters($toolproxy, $tool, $params, $parameters) {
    $retval = array();
    foreach ($parameters as $key => $val) {
        $key2 = lticustom_map_keyname($key);
        $val = lticustom_parse_custom_parameter($toolproxy, $tool, $params, $val, true);
        $retval['custom_'.$key2] = $val;
        if ($key != $key2) {
            $retval['custom_'.$key] = $val;
        }
 ...
 */
function lticustom_parse_custom_parameter($toolproxy, $tool, $params, $value, $islti2) {
    ...
    if ($value) {
        if (substr($value, 0, 1) == '\\') {
            ...
            $enabledcapabilities = lticustom_get_enabled_capabilities($tool);
            if (!$islti2 || in_array($value1, $enabledcapabilities)) {
                ...
                if (array_key_exists($value1, $capabilities)) {
                    ...
                    if ($val) {
                        ...
                        $value = lticustom_calculate_custom_parameter($value1);
                    }
                } else {
                    $val = $value;
 ...
 */
function lticustom_calculate_custom_parameter($value) {
    global $USER, $COURSE;
 ...
 */
function lticustom_map_keyname($key, $tolower = true) {
    if ($tolower) {
        $newkey = '';
 ...
 */
function lticustom_get_ims_role($user, $cmid, $courseid, $islti2) {
    $roles = array();
    ...
    if (empty($cmid)) {
        ...
        if (has_capability('mod/lticustom:manage', $context)) {
            array_push($roles, 'Instructor');
        } else {
            array_push($roles, 'Learner');
        }
    ...
    if (is_siteadmin($user) || has_capability('mod/lticustom:admin', $context)) {
 ...
 */
function lticustom_get_type_config($typeid) {
    global $DB;

    $query = "SELECT name, value
                FROM {lticustom_types_config}
               WHERE typeid = :typeid1
           UNION ALL
              SELECT 'toolurl' AS name, baseurl AS value
                FROM {lticustom_types}
               WHERE id = :typeid2
           UNION ALL
...
function lticustom_get_tools_by_url($url, $state, $courseid = null) {
    $domain = lticustom_get_domain_from_url($url);

    return lticustom_get_tools_by_domain($domain, $state, $courseid);
}

function lticustom_get_tools_by_domain($domain, $state = null, $courseid = null) {
    global $DB, $SITE;
 ...
 * @return array
 */
function lticustom_filter_get_types($course) {
    global $DB;
    ...
    $query = "SELECT t.id, t.name, t.baseurl, t.state, t.toolproxyid, t.timecreated, tp.name tpname
                FROM {lticustom_types} t LEFT OUTER JOIN {lticustom_tool_proxies} tp ON t.toolproxyid = tp.id
                {$where}";
 ...
 * @return array
 */
function lticustom_filter_tool_types(array $tools, $state) {
    $return = array();
 ...
 *        default [LTI_CUSTOM_COURSEVISIBLE_PRECONFIGURED, LTI_CUSTOM_COURSEVISIBLE_ACTIVITYCHOOSER]
 * @return stdClass[] All the lti types visible in the given course
 */
function lticustom_get_lti_types_by_course($courseid, $coursevisible = null) {
    global $DB, $SITE;

    if ($coursevisible === null) {
        $coursevisible = [LTI_CUSTOM_COURSEVISIBLE_PRECONFIGURED, LTI_CUSTOM_COURSEVISIBLE_ACTIVITYCHOOSER];
    }
    ...
    return $DB->get_records_sql($query,
        array('siteid' => $SITE->id, 'courseid' => $courseid, 'active' => LTI_CUSTOM_TOOL_STATE_CONFIGURED) + $coursevisparams);
}
 ...
 */
function lticustom_get_types_for_add_instance() {
    global $COURSE;
    $admintypes = lticustom_get_lti_types_by_course($COURSE->id);

    $types = array();
    $types[0] = (object)array('name' => get_string('automatic','lticustom'), 'course' => 0, 'toolproxyid' => null);
 ...
 */
function lticustom_get_configured_types($courseid, $sectionreturn = 0) {
    global $OUTPUT;
    $types = array();
    $admintypes = lticustom_get_lti_types_by_course($courseid, [LTI_CUSTOM_COURSEVISIBLE_ACTIVITYCHOOSER]);
    ...
    foreach ($admintypes as $ltitype) {
        ...
        if ($trimmeddescription != '') {
            ...
            $type->helplink = get_string('modulename_shortcut_link', 'lticustom');
        }
        if (empty($ltitype->icon)) {
            $type->icon = $OUTPUT->pix_icon('icon', '', 'lticustom', array('class' => 'icon'));
        } else {
            $type->icon = html_writer::empty_tag('img', array('src' => $ltitype->icon, 'alt' => $ltitype->name, 'class' => 'icon'));
        }
        $type->link = new moodle_url('/course/modedit.php', array('add' => 'lticustom', 'return' => 0, 'course' => $courseid,
...
function lticustom_get_domain_from_url($url) {
    $matches = array();

    if (preg_match(LTI_CUSTOM_URL_DOMAIN_REGEX, $url, $matches)) {
        return $matches[1];
    }
}

function lticustom_get_tool_by_url_match($url, $courseid = null, $state = LTI_CUSTOM_TOOL_STATE_CONFIGURED) {
    $possibletools = lticustom_get_tools_by_url($url, $state, $courseid);

    return lticustom_get_best_tool_by_url($url, $possibletools, $courseid);
}

function lticustom_get_url_thumbprint($url) {
...
function lticustom_get_best_tool_by_url($url, $tools, $courseid = null) {
    if (count($tools) === 0) {
        return null;
    }

    $urllower = lticustom_get_url_thumbprint($url);

    foreach ($tools as $tool) {
        $tool->_matchscore = 0;

        $toolbaseurllower = lticustom_get_url_thumbprint($tool->baseurl);
...
function lticustom_get_shared_secrets_by_key($key) {
    global $DB;
    ...
    $lti13 = LTI_CUSTOM_VERSION_1P3;
    $query = "SELECT t2.value
                FROM {lticustom_types_config} t1
                JOIN {lticustom_types_config} t2 ON t1.typeid = t2.typeid
                JOIN {lticustom_types} type ON t2.typeid = type.id
              ...
              SELECT tp.secret AS value
                FROM {lticustom_tool_proxies} tp
                JOIN {lticustom_types} t ON tp.id = t.toolproxyid
              WHERE tp.guid = :key2
    ...
    $sharedsecrets = $DB->get_records_sql($query, array('configured1' => LTI_CUSTOM_TOOL_STATE_CONFIGURED, 'ltiversion' => $lti13,
        'configured2' => LTI_CUSTOM_TOOL_STATE_CONFIGURED, 'key1' => $key, 'key2' => $key, 'key3' => $key));
 ...
 */
function lticustom_delete_type($id) {
    global $DB;

    // We should probably just copy the launch URL to the tool instances in this case... using a single query.
    /*
    $instances = $DB->get_records('lticustom', array('typeid' => $id));
    foreach ($instances as $instance) {
        $instance->typeid = 0;
        $DB->update_record('lticustom', $instance);
    }*/

    $DB->delete_records('lticustom_types', array('id' => $id));
    $DB->delete_records('lticustom_types_config', array('typeid' => $id));
}

function lticustom_set_state_for_type($id, $state) {
    global $DB;

    $DB->update_record('lticustom_types', (object)array('id' => $id, 'state' => $state));
}
 ...
 */
function lticustom_get_config($ltiobject) {
    $typeconfig = (array)$ltiobject;
    $additionalconfig = lticustom_get_type_config($ltiobject->typeid);
 ...
 *
 */
function lticustom_get_type_config_from_instance($id) {
    global $DB;

    $instance = $DB->get_record('lticustom', array('id' => $id));
    $config = lticustom_get_config($instance);
 ...
 */
function lticustom_get_type_type_config($id) {
    global $DB;

    $basicltitype = $DB->get_record('lticustom_types', array('id' => $id));
    ...
    foreach ($config as $name => $value) {
        if (strpos($name, 'lticustomservice_') === 0) {
...
function lticustom_prepare_type_for_save($type, $config) {
    if (isset($config->lti_toolurl)) {
        $type->baseurl = $config->lti_toolurl;
        $type->tooldomain = lticustom_get_domain_from_url($config->lti_toolurl);
    }
    ...
    if ((!empty($type->ltiversion) && $type->ltiversion === LTI_CUSTOM_VERSION_1P3) && empty($type->clientid)) {
...
function lticustom_update_type($type, $config) {
    global $DB, $CFG;

    lticustom_prepare_type_for_save($type, $config);

    if (lticustom_request_is_using_ssl() && !empty($type->secureicon)) {
    ...
    if ($DB->update_record('lticustom_types', $type)) {
        foreach ($config as $key => $value) {
            if (substr($key, 0, 4) == 'lti_' && !is_null($value)) {
                ...
                $record->value = $value;
                lticustom_update_config($record);
            }
            if (substr($key, 0, 17) == 'lticustomservice_' && !is_null($value)) {
                $record = new \StdClass();
                $record->typeid = $type->id;
                $record->name = $key;
                $record->value = $value;
...
function lticustom_add_type($type, $config) {
    global $USER, $SITE, $DB;

    lticustom_prepare_type_for_save($type, $config);

    if (!isset($type->state)) {
        $type->state = LTI_CUSTOM_TOOL_STATE_PENDING;
    }

    if (!isset($type->ltiversion)) {
        $type->ltiversion = LTI_CUSTOM_VERSION_1;
    }
    ...
    $id = $DB->insert_record('lticustom_types', $type);
    ...
    if ($id) {
        foreach ($config as $key => $value) {
            if (!is_null($value)) {
                ...
                } else if (substr($key, 0, 17) !== 'lticustomservice_') {
                    continue;
                } else {
                    $fieldname = $key;
                }
 ...
 *
 * @param array $toolproxies An array of lticustom_tool_proxies records
 * @param int $state One of the LTI_TOOL_PROXY_STATE_* constants
 *
 * @return array
 */
function lticustom_filter_tool_proxy_types(array $toolproxies, $state) {
    $return = array();
 ...
 * @return object
 */
function lticustom_get_tool_proxy_from_guid($toolproxyguid) {
    global $DB;

    $toolproxy = $DB->get_record('lticustom_tool_proxies', array('guid' => $toolproxyguid));
 ...
 */
function lticustom_get_tool_proxies_from_registration_url($regurl) {
    global $DB;

    return $DB->get_records_sql(
        'SELECT * FROM {lticustom_tool_proxies}
 ...
 */
function lticustom_get_tool_proxy($id) {
    global $DB;

    $toolproxy = $DB->get_record('lticustom_tool_proxies', array('id' => $id));
    return $toolproxy;
}
 ...
 */
function lticustom_get_tool_proxies($orphanedonly) {
    global $DB;

    if ($orphanedonly) {
        $usedproxyids = array_values($DB->get_fieldset_select('lticustom_types', 'toolproxyid', 'toolproxyid IS NOT NULL'));
        $proxies = $DB->get_records('lticustom_tool_proxies', null, 'state DESC, timemodified DESC');
        ...
        return $DB->get_records('lticustom_tool_proxies', null, 'state DESC, timemodified DESC');
    }
}
 ...
 * @return mixed  Tool Proxy details
 */
function lticustom_get_tool_proxy_config($id) {
    $toolproxy = lticustom_get_tool_proxy($id);
 ...
 */
function lticustom_add_tool_proxy($config) {
    global $USER, $DB;
    ...
    if (isset($config->lti_capabilities)) {
        ...
        $toolproxy->capabilityoffered = implode("\n", array_keys(lticustom_get_capabilities()));
    }
    if (isset($config->lti_services)) {
        ...
        $servicenames = array_map($func, lticustom_get_services());
        $toolproxy->serviceoffered = implode("\n", $servicenames);
    }
    if (isset($config->toolproxyid) && !empty($config->toolproxyid)) {
        $toolproxy->id = $config->toolproxyid;
        if (!isset($toolproxy->state) || ($toolproxy->state != LTI_CUSTOM_TOOL_PROXY_STATE_ACCEPTED)) {
            $toolproxy->state = LTI_CUSTOM_TOOL_PROXY_STATE_CONFIGURED;
            $toolproxy->guid = random_string();
            $toolproxy->secret = random_string();
        }
        $id = lticustom_update_tool_proxy($toolproxy);
    } else {
        $toolproxy->state = LTI_CUSTOM_TOOL_PROXY_STATE_CONFIGURED;
        ...
        $toolproxy->guid = random_string();
        $toolproxy->secret = random_string();
        $id = $DB->insert_record('lticustom_tool_proxies', $toolproxy);
    }
 ...
 * @return int    Record id number
 */
function lticustom_update_tool_proxy($toolproxy) {
    global $DB;

    $toolproxy->timemodified = time();
    $id = $DB->update_record('lticustom_tool_proxies', $toolproxy);
 ...
 */
function lticustom_delete_tool_proxy($id) {
    global $DB;
    $DB->delete_records('lticustom_tool_settings', array('toolproxyid' => $id));
    $tools = $DB->get_records('lticustom_types', array('toolproxyid' => $id));
    foreach ($tools as $tool) {
        lticustom_delete_type($tool->id);
    }
    $DB->delete_records('lticustom_tool_proxies', array('id' => $id));
}
 ...
 * @return int Record id number
 */
function lticustom_add_config($config) {
    global $DB;

    return $DB->insert_record('lticustom_types_config', $config);
}
 ...
 */
function lticustom_update_config($config) {
    global $DB;

    $old = $DB->get_record('lticustom_types_config', array('typeid' => $config->typeid, 'name' => $config->name));

    if ($old) {
        $config->id = $old->id;
        $return = $DB->update_record('lticustom_types_config', $config);
    } else {
        $return = $DB->insert_record('lticustom_types_config', $config);
    }
    return $return;
 ...
 */
function lticustom_get_tool_settings($toolproxyid, $courseid = null, $instanceid = null) {
    global $DB;

    $settings = array();
    if ($toolproxyid > 0) {
        $settingsstr = $DB->get_field('lticustom_tool_settings', 'settings', array('toolproxyid' => $toolproxyid,
            'course' => $courseid, 'coursemoduleid' => $instanceid));
    } else {
        $settingsstr = $DB->get_field('lticustom_tool_settings', 'settings', array('typeid' => -$toolproxyid,
            'course' => $courseid, 'coursemoduleid' => $instanceid));
    }
 ...
 * @param int    $instanceid    Id of course module (null if system or context settings)
 */
function lticustom_set_tool_settings($settings, $toolproxyid, $courseid = null, $instanceid = null) {
    global $DB;

    $json = json_encode($settings);
    if ($toolproxyid >= 0) {
        $record = $DB->get_record('lticustom_tool_settings', array('toolproxyid' => $toolproxyid,
            'course' => $courseid, 'coursemoduleid' => $instanceid));
    } else {
        $record = $DB->get_record('lticustom_tool_settings', array('typeid' => -$toolproxyid,
            'course' => $courseid, 'coursemoduleid' => $instanceid));
    }
    if ($record !== false) {
        $DB->update_record('lticustom_tool_settings', (object)array('id' => $record->id, 'settings' => $json, 'timemodified' => time()));
    } else {
        $record = new \stdClass();
        if ($toolproxyid > 0) {
        ...
        $DB->insert_record('lticustom_tool_settings', $record);
    }
}
 ...
 * @return array|null
 */
function lticustom_sign_parameters($oldparms, $endpoint, $method, $oauthconsumerkey, $oauthconsumersecret) {
 ...
 * @return array|null
 */
function lticustom_sign_jwt($parms, $endpoint, $oauthconsumerkey, $typeid = 0, $nonce = '') {
    global $CFG;

    if (empty($typeid)) {
        $typeid = 0;
    }
    $messagetypemapping = lticustom_get_jwt_message_type_mapping();
    ...
    $claimmapping = lticustom_get_jwt_claim_mapping();
    $payload = array(
        'nonce' => $nonce,
        'iat' => $now,
        'exp' => $now + 60,
    ...
    $payload[LTI_CUSTOM_JWT_CLAIM_PREFIX . '/claim/deployment_id'] = strval($typeid);
    $payload[LTI_CUSTOM_JWT_CLAIM_PREFIX . '/claim/target_link_uri'] = $endpoint;

    foreach ($parms as $key => $value) {
        $claim = LTI_CUSTOM_JWT_CLAIM_PREFIX;
    ...
    $privatekey = get_config('mod_lticustom', 'privatekey');
    $kid = get_config('mod_lticustom', 'kid');
 ...
 * @throws moodle_exception
 */
function lticustom_convert_from_jwt($typeid, $jwtparam) {
    ...
    if ($ok) {
        lticustom_verify_jwt_signature($typeid, $claims['iss'], $jwtparam);
        $params['oauth_consumer_key'] = $claims['iss'];
        foreach (lticustom_get_jwt_claim_mapping() as $key => $mapping) {
            $claim = LTI_CUSTOM_JWT_CLAIM_PREFIX;
            if (!empty($mapping['suffix'])) {
                $claim .= "-{$mapping['suffix']}";
            }
            ...
            $claim = LTI_CUSTOM_JWT_CLAIM_PREFIX . '/claim/custom';
            if (isset($claims[$claim])) {
                $custom = $claims[$claim];
                if (is_array($custom)) {
            ...
            $claim = LTI_CUSTOM_JWT_CLAIM_PREFIX . '/claim/ext';
            if (isset($claims[$claim])) {
                $ext = $claims[$claim];
                if (is_array($ext)) {
    ...
    if (isset($params['content_items'])) {
        $params['content_items'] = lticustom_convert_content_items($params['content_items']);
    }
    $messagetypemapping = lticustom_get_jwt_message_type_mapping();
 ...
 * @return string
 */
function lticustom_post_launch_html($newparms, $endpoint, $debug=false) {
 ...
 * @return string
 */
function lticustom_initiate_login($courseid, $id, $instance, $config, $messagetype = 'basic-lti-launch-request', $title = '',
        $text = '') {
...
function lticustom_get_type($typeid) {
    global $DB;

    return $DB->get_record('lticustom_types', array('id' => $typeid));
}

function lticustom_get_launch_container($lti, $toolconfig) {
    if (empty($lti->launchcontainer)) {
        $lti->launchcontainer = LTI_CUSTOM__LAUNCH_CONTAINER_DEFAULT;
    }

    if ($lti->launchcontainer == LTI_CUSTOM__LAUNCH_CONTAINER_DEFAULT) {
    ...
    if (empty($launchcontainer) || $launchcontainer == LTI_CUSTOM__LAUNCH_CONTAINER_DEFAULT) {
        $launchcontainer = LTI_CUSTOM_LAUNCH_CONTAINER_EMBED_NO_BLOCKS;
    }
    ...
    if ($devicetype === core_useragent::DEVICETYPE_MOBILE || $devicetype === core_useragent::DEVICETYPE_TABLET ) {
        $launchcontainer = LTI_CUSTOM_LAUNCH_CONTAINER_REPLACE_MOODLE_WINDOW;
    }

    return $launchcontainer;
}

function lticustom_request_is_using_ssl() {
    global $CFG;
    return (stripos($CFG->wwwroot, 'https://') === 0);
}

function lticustom_ensure_url_is_https($url) {
    if (!strstr($url, '://')) {
        $url = 'https://' . $url;
    } else {
 ...
 * @return bool
 */
function lticustom_should_log_request($rawbody) {
    global $CFG;

    if (empty($CFG->mod_lticustom_log_users)) {
        return false;
    }

    $logusers = explode(',', $CFG->mod_lticustom_log_users);
    if (empty($logusers)) {
        return false;
    }
 ...
 * @param string $rawbody
 */
function lticustom_log_request($rawbody) {
    if ($tempdir = make_temp_directory('mod_lticustom', false)) {
        if ($tempfile = tempnam($tempdir, 'mod_lticustom_request'.date('YmdHis'))) {
            $content  = "Request Headers:\n";
            foreach (moodle\mod\lticustom\OAuthUtil::get_headers() as $header => $value) {
 ...
 */
function lticustom_log_response($responsexml, $e = null) {
    if ($tempdir = make_temp_directory('mod_lticustom', false)) {
        if ($tempfile = tempnam($tempdir, 'mod_lticustom_response'.date('YmdHis'))) {
            $content = '';
 ...
 */
function lticustom_get_type_config_by_instance($instance) {
    $typeid = null;
    if (empty($instance->typeid)) {
        $tool = lticustom_get_tool_by_url_match($instance->toolurl, $instance->course);
        if ($tool) {
    ...
    if (!empty($typeid)) {
        return lticustom_get_type_config($typeid);
    }
    return array();
}
 ...
 */
function lticustom_force_type_config_settings($instance, array $typeconfig) {
    $forced = array(
    ...
    foreach ($forced as $instanceparam => $typeconfigparam) {
        if (array_key_exists($typeconfigparam, $typeconfig) && $typeconfig[$typeconfigparam] != LTI_CUSTOM_SETTING_DELEGATE) {
 ...
 */
function lticustom_get_capabilities() {
 ...
 */
function lticustom_get_services() {

    $services = array();
    $definedservices = core_component::get_plugin_list('lticustomservice');

    foreach ($definedservices as $name => $location) {
        $classname = "\\lticustomservice_{$name}\\local\\service\\{$name}";
 ...
 *
 * @return bool|\mod_lticustom\local\ltiservice\service_base Service
 */
function lticustom_get_service_by_name($servicename) {

    $service = false;
    $classname = "\\lticustomservice_{$servicename}\\local\\service\\{$servicename}";
 ...
 *
 * @param \mod_lticustom\local\ltiservice\service_base[] $services Array of services
 * @param string $resourceid  ID of resource
 *
 * @return mod_lticustom\local\ltiservice\service_base Service
 */
function lticustom_get_service_by_resource_id($services, $resourceid) {
 ...
 */
function lticustom_get_permitted_service_scopes($type, $typeconfig) {
 ...
 */
function lticustom_get_contexts($json) {
 ...
 */
function lticustom_get_fqid($contexts, $id) {
 ...
 */
function lticustom_get_tool_type_icon_url(stdClass $type) {
    global $OUTPUT;
    ...
    if (empty($iconurl)) {
        $iconurl = $OUTPUT->image_url('icon', 'lticustom')->out();
    }
 ...
 */
function lticustom_get_tool_type_edit_url(stdClass $type) {
    $url = new moodle_url('/mod/lticustom/typessettings.php',
 ...
 * @return string The url to edit the tool type
 */
function lticustom_get_tool_proxy_edit_url(stdClass $proxy) {
    $url = new moodle_url('/mod/lticustom/registersettings.php',
 ...
 */
function lticustom_get_tool_type_course_url(stdClass $type) {
    if ($type->course != 1) {
 ...
 */
function lticustom_get_tool_type_urls(stdClass $type) {
    $courseurl = lticustom_get_tool_type_course_url($type);

    $urls = array(
        'icon' => lticustom_get_tool_type_icon_url($type),
        'edit' => lticustom_get_tool_type_edit_url($type),
    );
    ...
    $url = new moodle_url('/mod/lticustom/certs.php');
    $urls['publickeyset'] = $url->out();
    $url = new moodle_url('/mod/lticustom/token.php');
    $urls['accesstoken'] = $url->out();
    $url = new moodle_url('/mod/lticustom/auth.php');
 ...
 */
function lticustom_get_tool_proxy_urls(stdClass $proxy) {
    global $OUTPUT;

    $urls = array(
        'icon' => $OUTPUT->image_url('icon', 'lticustom')->out(),
        'edit' => lticustom_get_tool_proxy_edit_url($proxy),
    );
 ...
 */
function lticustom_get_tool_type_state_info(stdClass $type) {
    $isconfigured = false;
    $ispending = false;
    $isrejected = false;
    $isunknown = false;
    switch ($type->state) {
        case LTI_CUSTOM_TOOL_STATE_CONFIGURED:
            $state = get_string('active', 'mod_lticustom');
            $isconfigured = true;
            break;
        case LTI_CUSTOM_TOOL_STATE_PENDING:
            $state = get_string('pending', 'mod_lticustom');
            $ispending = true;
            break;
        case LTI_CUSTOM_TOOL_STATE_REJECTED:
            $state = get_string('rejected', 'mod_lticustom');
            $isrejected = true;
            break;
        default:
            $state = get_string('unknownstate', 'mod_lticustom');
            $isunknown = true;
            break;
    }
 ...
 */
function lticustom_get_tool_type_config($type) {
    global $CFG;
    $platformid = $CFG->wwwroot;
    $clientid = $type->clientid;
    $deploymentid = $type->id;
    $publickeyseturl = new moodle_url('/mod/lticustom/certs.php');
    $publickeyseturl = $publickeyseturl->out();

    $accesstokenurl = new moodle_url('/mod/lticustom/token.php');
    $accesstokenurl = $accesstokenurl->out();

    $authrequesturl = new moodle_url('/mod/lticustom/auth.php');
 ...
 */
function lticustom_get_tool_type_capability_groups($type) {
    $capabilities = lticustom_get_enabled_capabilities($type);
    $groups = array();
    $hascourse = false;
    $hasactivities = false;
    ...
    foreach ($capabilities as $capability) {
        ...
        if (!$hascourse && preg_match('/^CourseSection/', $capability)) {
            $hascourse = true;
            $groups[] = get_string('courseinformation', 'mod_lticustom');
        } else if (!$hasactivities && preg_match('/^ResourceLink/', $capability)) {
            $hasactivities = true;
            $groups[] = get_string('courseactivitiesorresources', 'mod_lticustom');
        } else if (!$hasuseraccount && preg_match('/^User/', $capability) || preg_match('/^Membership/', $capability)) {
            $hasuseraccount = true;
            $groups[] = get_string('useraccountinformation', 'mod_lticustom');
        } else if (!$hasuserpersonal && preg_match('/^Person/', $capability)) {
            $hasuserpersonal = true;
            $groups[] = get_string('userpersonalinformation', 'mod_lticustom');
        }
    }
 ...
 */
function lticustom_get_tool_type_instance_ids($type) {
    global $DB;

    return array_keys($DB->get_fieldset_select('lticustom', 'id', 'typeid = ?', array($type->id)));
}
 ...
 */
function lticustom_serialise_tool_type(stdClass $type) {
    global $CFG;

    $capabilitygroups = lticustom_get_tool_type_capability_groups($type);
    $instanceids = lticustom_get_tool_type_instance_ids($type);
    // Clean the name. We don't want tags here.
    ...
    if (!empty($type->description)) {
        ...
        $description = get_string('editdescription', 'mod_lticustom');
    }
    return array(
        'id' => $type->id,
        'name' => $name,
        'description' => $description,
        'urls' => lticustom_get_tool_type_urls($type),
        'state' => lticustom_get_tool_type_state_info($type),
        'platformid' => $CFG->wwwroot,
        'clientid' => $type->clientid,
        'deploymentid' => $type->id,
 ...
 * @return array An array of values representing this type
 */
function lticustom_serialise_tool_proxy(stdClass $proxy) {
    return array(
        'id' => $proxy->id,
        'name' => $proxy->name,
        'description' => get_string('activatetoadddescription', 'mod_lticustom'),
        'urls' => lticustom_get_tool_proxy_urls($proxy),
        'state' => array(
            'text' => get_string('pending', 'mod_lticustom'),
            'pending' => true,
            'configured' => false,
            'rejected' => false,
            'unknown' => false
 ...
 * @since Moodle 3.1
 */
function lticustom_load_type_if_cartridge($type) {
    if (!empty($type->lti_toolurl) && lticustom_is_cartridge($type->lti_toolurl)) {
        lticustom_load_type_from_cartridge($type->lti_toolurl, $type);
    }
}
 ...
 * @since Moodle 3.1
 */
function lticustom_load_tool_if_cartridge($lti) {
    if (!empty($lti->toolurl) && lticustom_is_cartridge($lti->toolurl)) {
        lticustom_load_tool_from_cartridge($lti->toolurl, $lti);
    }
}
 ...
 * @since Moodle 3.1
 */
function lticustom_is_cartridge($url) {
    ...
    try {
        $toolinfo = lticustom_load_cartridge($url,
            array(
 ...
 * @since Moodle 3.1
 */
function lticustom_load_type_from_cartridge($url, $type) {
    $toolinfo = lticustom_load_cartridge($url,
        array(
            "title" => "lti_typename",
 ...
 * @since Moodle 3.1
 */
function lticustom_load_tool_from_cartridge($url, $lti) {
    $toolinfo = lticustom_load_cartridge($url,
        array(
            "title" => "name",
            "launch_url" => "toolurl",
 ...
 * @since Moodle 3.1
 */
function lticustom_load_cartridge($url, $map, $propertiesmap = array()) {
    global $CFG;
    ...
    foreach ($map as $tag => $key) {
        $value = lticustom_get_tag($tag, $cartridge);
        if ($value) {
            $toolinfo[$key] = $value;
        }
    }
    if (!empty($propertiesmap)) {
        foreach ($propertiesmap as $property => $key) {
            $value = lticustom_get_tag("property", $cartridge, $property);
            if ($value) {
                $toolinfo[$key] = $value;
            }
        }
 ...
 * @since Moodle 3.1
 */
function lticustom_get_tag($tagname, $xpath, $attribute = null) {
    if ($attribute) {
 ...
 */
function lticustom_new_access_token($typeid, $scopes) {
    global $DB;
    ...
    } while ($DB->record_exists('lticustom_access_tokens', array('token' => $generatedtoken)));
    ...
    $newtoken->validuntil = $newtoken->timecreated + LTI_CUSTOM_ACCESS_TOKEN_LIFE;
    $newtoken->lastaccess = null;

    $DB->insert_record('lticustom_access_tokens', $newtoken);
