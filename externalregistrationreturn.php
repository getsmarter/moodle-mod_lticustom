...
$pageurl = new moodle_url('/mod/lticustom/externalregistrationreturn.php');
...
if ($status !== 'success' && empty($err)) {
    ...
    if (!empty($msg)) {
        ...
        $err = get_string('failedtocreatetooltype', 'mod_lticustom');
    }
}
$params = array('message' => s($msg), 'error' => s($err), 'id' => $id, 'status' => s($status));

$page = new \mod_lticustom\output\external_registration_return_page();
echo $output->render($page);

$PAGE->requires->js_call_amd('mod_lticustom/external_registration_return', 'init', $params);
echo $output->footer();
