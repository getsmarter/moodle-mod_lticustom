 ...
 *
 * @package    lticustomservice_toolproxy
...
namespace lticustomservice_toolproxy\local\resources;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class toolproxy extends \mod_lticustom\local\ltiservice\resource_base {

    /**
     * Class constructor.
     *
     * @param lticustomservice_toolproxy\local\resources\toolproxy $service Service instance
     */
     ...
     *
     * @param mod_lticustom\local\ltiservice\response $response  Response object for this request.
     */
    public function execute($response) {
        ...
        if ($ok && isset($toolproxyjson->security_contract->tool_service)) {
            $contexts = lticustom_get_contexts($toolproxyjson);
            ...
            foreach ($tpservices as $service) {
                $fqid = lticustom_get_fqid($contexts, $service->service);
                ...
                if (substr($fqid, 0, strlen($context)) !== $context) {
                    ...
                    $id = explode('#', $fqid, 2);
                    $aservice = lticustom_get_service_by_resource_id($services, $id[1]);
        ...
        if ($ok) {
            ...
            foreach ($tools as $tool) {
                ...
                $type->ltiversion = LTI_CUSTOM_VERSION_2;
                ...
                $ok = $ok && (lticustom_add_type($type, $config) !== false);
            }
            if (isset($toolproxyjson->custom)) {
                lticustom_set_tool_settings($toolproxyjson->custom, $toolproxy->id);
            }
        }
        ...
        if (!empty($toolproxy)) {
            if ($ok) {
                ...
                $toolproxy->state = LTI_CUSTOM_TOOL_PROXY_STATE_ACCEPTED;
...
EOD;
                ...
                $toolproxy->state = LTI_CUSTOM_TOOL_PROXY_STATE_REJECTED;
