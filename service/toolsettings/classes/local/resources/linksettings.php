...
namespace lticustomservice_toolsettings\local\resources;

use lticustomservice_toolsettings\local\service\toolsettings;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class linksettings extends \mod_lticustom\local\ltiservice\resource_base {

    /**
     * Class constructor.
     *
     * @param \mod_lticustom\local\ltiservice\service_base $service Service instance
     */
    ...
    public function execute($response) {
        ...
        if ($ok) {
            $ok = !empty($linkid);
            if ($ok) {
                $lti = $DB->get_record('lticustom', array('id' => $linkid), 'course,typeid', MUST_EXIST);
        ...
        if ($ok) {
            ...
            if ($response->get_request_method() == 'GET') {
                $linksettings = lticustom_get_tool_settings($id, $lti->course, $linkid);
                if (!empty($bubble)) {
                    ...
                    $contextsettings = lticustom_get_tool_settings($id, $lti->course);
                    ...
                    $systemsettings = lticustom_get_tool_settings($id);
                    if ($bubble == 'distinct') {
                ...
                if ($ok) {
                    lticustom_set_tool_settings($settings, $id, $lti->course, $linkid);
                } else {
                    $response->set_code(406);
                }
            }
