 ...
 *
 * @package    lticustomservice_profile
...
namespace lticustomservice_profile\local\resources;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class profile extends \mod_lticustom\local\ltiservice\resource_base {
    ...
    public function execute($response) {
        ...
        if (optional_param('lti_version', service_base::LTI_VERSION2P0, PARAM_ALPHANUMEXT) != $version) {
            ...
            $toolproxy = lticustom_get_tool_proxy_from_guid($params['tool_proxy_id']);
            $ok = $toolproxy !== false;
        }
        if ($ok) {
            ...
            $serviceoffered = '';
            $sep = '';
            $services = \core_component::get_plugin_list('lticustomservice');
...
EOD;
            ...
            if (!empty($CFG->mod_lticustom_institution_name)) {
                $consumername = $CFG->mod_lticustom_institution_name;
                $consumerdesc = '';
            } else {
