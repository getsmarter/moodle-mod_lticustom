...
namespace lticustomservice_memberships\local\resources;

use mod_lticustom\local\ltiservice\resource_base;
use lticustomservice_memberships\local\service\memberships;
...
class linkmemberships extends resource_base {
     ...
     * Class constructor.
     *
     * @param \lticustomservice_memberships\local\service\memberships $service Service instance
     */
    ...
    public function execute($response) {
        ...
        if (!($lti = $DB->get_record('lticustom', array('id' => $linkid), 'id,course,typeid,servicesalt', IGNORE_MISSING))) {
            $response->set_code(404);
            return;
        }
        ...
        $cm = get_coursemodule_from_instance('lticustom', $linkid, $lti->course, false, MUST_EXIST);
