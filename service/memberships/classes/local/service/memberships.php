 ...
 *
 * @package    lticustomservice_memberships
 * @copyright  2015 Vital Source Technologies http://vitalsource.com
 * @author     Stephen Vickers
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace lticustomservice_memberships\local\service;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class memberships extends \mod_lticustom\local\ltiservice\service_base {
    ...
    public function get_resources() {
        ...
        if (empty($this->resources)) {
            ...
            $this->resources[] = new \lticustomservice_memberships\local\resources\contextmemberships($this);
            $this->resources[] = new \lticustomservice_memberships\local\resources\linkmemberships($this);
        }
     ...
     *
     * @param \mod_lticustom\local\ltiservice\resource_base $resource       Resource handling the request
    ...
    public static function get_users_json($resource, $context, $contextid, $tool, $role, $limitfrom, $limitnum, $lti, $info) {
        ...
        $response = new \mod_lticustom\local\ltiservice\response();
     ...
     * @param \core_availability\info_module $info Conditional availability information
     *      for LTI instance (null if context-level request)
     * @param \mod_lticustom\local\ltiservice\response $response       Response object for the request
     *
     * @return string
     */
    public function get_members_json($resource, $context, $course, $role, $limitfrom, $limitnum, $lti, $info, $response) {
        ...
        if (($response->get_accept() === 'application/vnd.ims.lti-nrps.v2.membershipcontainer+json') ||
            ...
            ($this->get_type()->ltiversion === LTI_CUSTOM_VERSION_1P3))) {
    ...
    private function users_to_jsonld($resource, $users, $contextid, $exclude, $limitfrom, $limitnum,
        ...
        foreach ($users as $user) {
            ...
            $membership->role = explode(',', lticustom_get_ims_role($user->id, null, $contextid, true));

            $instanceconfig = null;
            if (!is_null($lti)) {
                $instanceconfig = lticustom_get_type_config_from_instance($lti->id);
            }
            ...
            if (!is_null($lti)) {
                ...
                if (!empty($lti->servicesalt) && $DB->record_exists('grade_items', $conditions)) {
                    $message->lis_result_sourcedid = json_encode(lticustom_build_sourcedid($lti->id,
                                                                                     $user->id,
                                                                                     $lti->servicesalt,
                                                                                     $lti->typeid));
    ...
    private function users_to_json($resource, $users, $course, $exclude, $limitfrom, $limitnum,
        ...
        foreach ($users as $user) {
            ...
            $member->roles = explode(',', lticustom_get_ims_role($user->id, null, $course->id, true));
            ...
            if (!is_null($lti)) {
                ...
                if (!empty($lti->servicesalt) && $DB->record_exists('grade_items', $conditions)) {
                    ...
                    $basicoutcome->lis_result_sourcedid = json_encode(lticustom_build_sourcedid($lti->id,
                                                                                     $user->id,
                                                                                     $lti->servicesalt,
                                                                                     $lti->typeid));
    ...
    public function get_launch_parameters($messagetype, $courseid, $user, $typeid, $modlti = null) {
        ...
        $tool = lticustom_get_type_type_config($typeid);
