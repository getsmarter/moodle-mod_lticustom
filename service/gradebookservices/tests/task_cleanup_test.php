 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class lticustomservice_gradebookservices_cleanup_task_testcase extends advanced_testcase {
    ...
    public function test_cleanup_task() {
        ...
        $lti = $this->getDataGenerator()->create_module('lticustom', ['course' => $course->id]);
        $lti2 = $this->getDataGenerator()->create_module('lticustom', ['course' => $course->id]);

        $conditions = [
            'courseid' => $course->id,
            'itemtype' => 'mod',
            'itemmodule' => 'lticustom',
            'iteminstance' => $lti->id
        ];
        ...
        // Insert these into the 'lticustom_gradebookservices' table.
        $data = new stdClass();
        $data->gradeitemid = $gradeitem->id;
        $data->courseid = $course->id;
        $DB->insert_record('lticustom_gradebookservices', $data);

        $data->gradeitemid = $gradeitem2->id;
        $DB->insert_record('lticustom_gradebookservices', $data);

        $task = new \lticustomservice_gradebookservices\task\cleanup_task();
        $task->execute();

        // Check they both still exist.
        $this->assertEquals(2, $DB->count_records('lticustom_gradebookservices'));
        ...
        $gradebookserviceitems = $DB->get_records('lticustom_gradebookservices');
    ...
    public function test_cleanup_task_with_manual_item() {
        ...
        // Insert it into the 'lticustom_gradebookservices' table.
        $data = new stdClass();
        $data->gradeitemid = $gradeitem->id;
        $data->courseid = $course->id;
        $DB->insert_record('lticustom_gradebookservices', $data);
        ...
        $this->assertEquals(1, $DB->count_records('lticustom_gradebookservices'));
        ...
        $this->assertEquals(0, $DB->count_records('lticustom_gradebookservices'));
    }
}
