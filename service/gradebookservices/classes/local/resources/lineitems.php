...
class lineitems extends resource_base {
     ...
     * Class constructor.
     *
     * @param \lticustomservice_gradebookservices\local\service\gradebookservices $service Service instance
     */
     ...
     * @param \mod_lticustom\local\ltiservice\response $response
    ...
    private function get_json_for_post_request($body, $contextid, $typeid) {
        ...
        $item->grademax = $max;
        $id = $item->insert('mod/lticustomservice_gradebookservices');
        $DB->insert_record('lticustom_gradebookservices', (object)array(
                'gradeitemid' => $id,
                'courseid' => $contextid,
