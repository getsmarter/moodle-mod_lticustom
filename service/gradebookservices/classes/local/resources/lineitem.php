...
namespace lticustomservice_gradebookservices\local\resources;

use lticustomservice_gradebookservices\local\service\gradebookservices;
...
class lineitem extends resource_base {
     ...
     * @param \lticustomservice_gradebookservices\local\resources\lineitem $olditem Grade item instance
     * @param string $typeid Tool Type Id
     *
     * @return string
     * @throws \Exception
    ...
    private function process_put_request($body, $olditem, $typeid) {
        ...
        if ($updategradeitem) {
            if (!$item->update('mod/lticustomservice_gradebookservices')) {
        ...
        if ($upgradegradebookservices) {
            if (is_null($typeid)) {
                ...
                $toolproxyid = null;
                $baseurl = lticustom_get_type_type_config($typeid)->lti_toolurl;
            }
            $DB->update_record('lticustom_gradebookservices', (object)array(
                    'id' => $gbs->id,
     ...
     *
     * @param \lticustomservice_gradebookservices\local\resources\lineitem $item Grade item instance
     * @throws \Exception
     */
    private function process_delete_request($item) {
        ...
        if (!$gradeitem->delete('mod/lticustomservice_gradebookservices')) {
            throw new \Exception(null, 500);
        } else {
            $sqlparams = array();
            $sqlparams['id'] = $gbs->id;
            if (!$DB->delete_records('lticustom_gradebookservices', $sqlparams)) {
                throw new \Exception(null, 500);
            }
        }
    }
    ...
    public function parse_value($value) {
        ...
        if (strpos($value, '$LineItem.url') !== false) {
            ...
            if (!empty($id)) {
                ...
                $id = $cm->instance;
                $item = grade_get_grades($COURSE->id, 'mod', 'lticustom', $id);
                if ($item && $item->items) {
