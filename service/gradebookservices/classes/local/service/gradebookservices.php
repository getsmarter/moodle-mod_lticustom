 ...
 *
 * @package    lticustomservice_gradebookservices
 * @copyright  2017 Cengage Learning http://www.cengage.com
 * @author     Dirk Singels, Diego del Blanco, Claude Vervoort
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace lticustomservice_gradebookservices\local\service;

use lticustomservice_gradebookservices\local\resources\lineitem;
use lticustomservice_gradebookservices\local\resources\lineitems;
use lticustomservice_gradebookservices\local\resources\results;
use lticustomservice_gradebookservices\local\resources\scores;
use mod_lticustom\local\ltiservice\resource_base;
use mod_lticustom\local\ltiservice\service_base;
...
class gradebookservices extends service_base {
    ...
    public function get_launch_parameters($messagetype, $courseid, $user, $typeid, $modlti = null) {
        ...
        $this->set_type(lticustom_get_type($typeid));
        $this->set_typeconfig(lticustom_get_type_config($typeid));
                ...
                if ($this->is_allowed_in_context($typeid, $courseid)) {
                    if (is_null($modlti)) {
                        ...
                        $conditions = array('courseid' => $courseid, 'itemtype' => 'mod',
                                'itemmodule' => 'lticustom', 'iteminstance' => $modlti);

                        $lineitems = $DB->get_records('grade_items', $conditions);
                        $conditionsgbs = array('courseid' => $courseid, 'ltilinkid' => $modlti);
                        $lineitemsgbs = $DB->get_records('lticustom_gradebookservices', $conditionsgbs);
    ...
    public function get_lineitems($courseid, $resourceid, $ltilinkid, $tag, $limitfrom, $limitnum, $typeid) {
        ...
        if ($lineitems) {
            foreach ($lineitems as $lineitem) {
                ...
                if ($gbs && (!isset($tag) || (isset($tag) && $gbs->tag == $tag))
                ...
                } else if (($lineitem->itemtype == 'mod') && ($lineitem->itemmodule == 'lticustom') && (!isset($tag) &&
                    ...
                    $ltiactivity = $DB->get_record('lticustom', array('id' => $lineitem->iteminstance));
                    if (($ltiactivity) && (isset($ltiactivity->typeid))) {
                        if ($ltiactivity->typeid != 0) {
                            $tool = $DB->get_record('lticustom_types', array('id' => $ltiactivity->typeid));
                        } else {
                            $tool = lticustom_get_tool_by_url_match($ltiactivity->toolurl, $courseid);
                            if (!$tool) {
                                $tool = lticustom_get_tool_by_url_match($ltiactivity->securetoolurl, $courseid);
                            }
                        }
                        if (is_null($typeid)) {
     ...
     * @param string $typeid
     *
     * @return \lticustomservice_gradebookservices\local\resources\lineitem|bool
     */
    ...
    public static function check_lti_id($linkid, $course, $toolproxy) {
        ...
        $ltiactivity = $DB->get_record('lticustom', array('id' => $linkid, 'course' => $course));
        if ($ltiactivity->typeid == 0) {
            $tool = lticustom_get_tool_by_url_match($ltiactivity->toolurl, $course);
            if (!$tool) {
                $tool = lticustom_get_tool_by_url_match($ltiactivity->securetoolurl, $course);
            }
            ...
            $sql = 'SELECT lti.*
                      FROM {lticustom} lti
                INNER JOIN {lticustom_types} typ ON lti.typeid = typ.id
                     WHERE lti.id = ?
                           AND lti.course = ?
                           AND typ.toolproxyid = ?';
    ...
    /**
     * Deletes orphaned rows from the 'lticustom_gradebookservices' table.
     *
     * Sometimes, if a gradebook entry is deleted and it was a lineitem
     * the row in the table lticustomservice_gradebookservices can become an orphan
     ...
     */
    public static function delete_orphans_lticustomservice_gradebookservices_rows() {
        global $DB;

        $sql = "DELETE
                  FROM {lticustom_gradebookservices}
    ...
    public static function find_ltiservice_gradebookservice_for_lineitem($lineitemid) {
        ...
        if ($gradeitem) {
            $gbs = $DB->get_record('lticustom_gradebookservices',
