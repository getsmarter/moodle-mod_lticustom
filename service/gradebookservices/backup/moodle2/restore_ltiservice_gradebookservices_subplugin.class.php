 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_lticustomservice_gradebookservices_subplugin extends restore_subplugin {
     ...
     * @return void
     */
    public function process_lticustomservice_gradebookservices_lineitem($data) {
        global $DB;
        $data = (object)$data;
        // The coupled lineitems are restored as any other grade item
        // so we will only create the entry in the lticustomservice_gradebookservices table.
        ...
        if ($data->ltilinkid != null) {
            $ltilinkid = $this->get_new_parentid('lticustom');
        } else {
            $ltilinkid = null;
        }
        // If this has not been restored before.
        if ($this->get_mappingid('gbsgradeitemrestored',  $data->id, 0) == 0) {
            $newgbsid = $DB->insert_record('lticustom_gradebookservices', (object) array(
                    'gradeitemid' => 0,
                    'courseid' => $courseid,
    ...
    private function find_proxy_id($data) {
        ...
        $dbtoolproxy = $DB->get_field('lticustom_tool_proxies', 'id', $dbtoolproxyjsonparams, IGNORE_MISSING);
        if ($dbtoolproxy) {
    ...
    private function find_typeid($data, $courseid) {
        ...
        $dbtype = $DB->get_field_select('lticustom_types', 'id', "id=:id
        ...
        if ($dbtype) {
            ...
            if ($dbtype) {
                ...
                $dbtype = $DB->get_field_select('lticustom_types', 'id', "course=:course
                ...
                if ($dbtype) {
                    ...
                    $dbtype = $DB->get_field_select('lticustom_types', 'id', "course=1
                            AND ".$DB->sql_compare_text('baseurl')."=:baseurl",
                            $dbtypeidparameter);
                    if ($dbtype) {
                        $newtypeid = $dbtype;
    ...
    protected function after_restore_lti() {
        ...
        $activitytask = $this->task;
        $courseid = $activitytask->get_courseid();
        $gbstoupdate = $DB->get_records('lticustom_gradebookservices', array('gradeitemid' => 0, 'courseid' => $courseid));
        foreach ($gbstoupdate as $gbs) {
            ...
            if ($newgradeitemid > 0) {
                ...
                $DB->update_record('lticustom_gradebookservices', $gbs);
            }
        }
    }
