 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class backup_lticustomservice_gradebookservices_subplugin extends backup_subplugin {
    ...
    protected function define_lti_subplugin_structure() {
        ...
        $lti = $DB->get_record('lticustom', ['id' => $activityid], 'typeid, toolurl, securetoolurl');
        $ltitype = $DB->get_record('lticustom_types', ['id' => $lti->typeid], 'toolproxyid, baseurl');
        if ($ltitype) {
            $typeid = $lti->typeid;
            ...
            $gbsrecord = $DB->get_record('lticustom_gradebookservices',
            ...
            if ($gbsrecord) {
                ...
                $tool = lticustom_get_tool_by_url_match($lti->toolurl, $activitycourseid);
                if (!$tool) {
                    $tool = lticustom_get_tool_by_url_match($lti->securetoolurl, $activitycourseid);
                }
                if ($tool) {
                    $alttypeid = $tool->id;
                    // If we have a valid typeid then get types again.
                    if ($alttypeid != self::NONVALIDTYPEID) {
                        $ltitype = $DB->get_record('lticustom_types', ['id' => $alttypeid], 'toolproxyid, baseurl');
                        $toolproxyid = $ltitype->toolproxyid;
                    }
                }
            }
        ...
        if ($toolproxyid != null) {
            $lineitemssql = "SELECT l.*, t.vendorcode as vendorcode, t.guid as guid
                               FROM {lticustom_gradebookservices} l
                         INNER JOIN {lticustom_tool_proxies} t ON (t.id = l.toolproxyid)
                              WHERE l.courseid = ?
                                AND l.toolproxyid = ?
