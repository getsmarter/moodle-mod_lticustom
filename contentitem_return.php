...
if (!empty($jwt)) {
    $params = lticustom_convert_from_jwt($id, $jwt);
    ...
    lticustom_verify_oauth_signature($id, $consumerkey);
}
...
if (empty($errormsg) && !empty($items)) {
    try {
        $returndata = lticustom_tool_configuration_from_content_item($id, $messagetype, $version, $consumerkey, $items);
    } catch (moodle_exception $e) {
        $errormsg = $e->getMessage();
    }
}

$pageurl = new moodle_url('/mod/lticustom/contentitem_return.php');
$PAGE->set_url($pageurl);
$PAGE->set_pagelayout('popup');
echo $OUTPUT->header();

// Call JS module to redirect the user to the course page or close the dialogue on error/cancel.
$PAGE->requires->js_call_amd('mod_lticustom/contentitem_return', 'init', [$returndata]);
...
if ($errormsg) {
    ...
    // Means success.
    if (!$msg) {
        $msg = get_string('successfullyfetchedtoolconfigurationfromcontent','lticustom');
    }
