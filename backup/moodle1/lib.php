...
class moodle1_mod_lticustom_handler extends moodle1_mod_handler {
    ...
    public function process_basiclticustom($data) {
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_lticustom');
        ...
        if (!$DB->record_exists('lticustom_types', array('id' => $data['typeid']))) {
            ...
            if ($ntypeid === false) {
                $ntypeid = $DB->get_field('lticustom_types_config',
                                          'typeid',
                                          array(),
                                          IGNORE_MULTIPLE);
    ...
    public function on_basiclticustom_end() {
        ...
        // Write inforef.xml.
        $this->open_xml_writer("activities/lticustom_{$this->moduleid}/inforef.xml");
