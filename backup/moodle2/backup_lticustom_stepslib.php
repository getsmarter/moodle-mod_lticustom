 ...
 */
class backup_lticustom_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        // Define sources.
        $ltirecord = $DB->get_record('lticustom', ['id' => $this->task->get_activityid()]);
        ...
        if ($userinfo) {
            $ltisubmission->set_source_table('lticustom_submission', array('ltiid' => backup::VAR_ACTIVITYID));
        }
        ...
        $lti->annotate_files('mod_lticustom', 'intro', null); // This file areas haven't itemid.

        // Add support for subplugin structures.
        $this->add_subplugin_structure('lticustomsource', $lti, true);
        $this->add_subplugin_structure('lticustomservice', $lti, true);
     ...
     *
     * @param stdClass $ltirecord record from {lticustom} table
    ...
    protected function retrieve_lti_type($ltirecord) {
        ...
        $record = $DB->get_record('lticustom_types', ['id' => $ltirecord->typeid]);
