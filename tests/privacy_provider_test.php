...
use mod_lticustom\privacy\provider;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_lticustom_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {

    /**
     * Test for provider::get_metadata().
     */
    public function test_get_metadata() {
        $collection = new collection('mod_lticustom');
        ...
        $this->assertEquals('lticustom_submission', $ltisubmissiontable->get_name());

        $lticustomtoolproxies = array_shift($itemcollection);
        $this->assertEquals('lticustom_tool_proxies', $lticustomtoolproxies->get_name());

        $ltitypestable = array_shift($itemcollection);
        $this->assertEquals('lticustom_types', $ltitypestable->get_name());
        ...
        $privacyfields = $lticustomtoolproxies->get_privacy_fields();
        ...
        $this->assertEquals('privacy:metadata:lticustom_tool_proxies', $lticustomtoolproxies->get_summary());
    ...
    public function test_get_contexts_for_userid() {
        ...
        $this->getDataGenerator()->create_module('lticustom', array('course' => $course->id));
    ...
    public function test_get_users_in_context() {
        ...
        $course = $this->getDataGenerator()->create_course();
        $component = 'mod_lticustom';

        // The LTI activity the user will have submitted something for.
        $lti1 = $this->getDataGenerator()->create_module('lticustom', array('course' => $course->id));

        // Another LTI activity that has no user activity.
        $lti2 = $this->getDataGenerator()->create_module('lticustom', array('course' => $course->id));
    ...
    public function test_export_for_context_submissions() {
        ...
        $cmcontext = context_module::instance($lti->cmid);
        $this->export_context_data_for_user($user1->id, $cmcontext, 'mod_lticustom');
    ...
    public function test_export_for_context_tool_types() {
        ...
        lticustom_add_type($type, new stdClass());
        ...
        $this->export_context_data_for_user($user->id, $coursecontext, 'mod_lticustom');
    ...
    public function test_export_for_context_tool_proxies() {
        ...
        lticustom_add_tool_proxy($toolproxy);

        // Export all of the data for the context.
        $systemcontext = context_system::instance();
        $this->export_context_data_for_user($user->id, $systemcontext, 'mod_lticustom');
        $writer = \core_privacy\local\request\writer::with_context($systemcontext);

        $this->assertTrue($writer->has_any_data());

        $data = $writer->get_data();
        $this->assertCount(1, $data->lticustom_tool_proxies);
    }
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $count = $DB->count_records('lticustom_submission', ['ltiid' => $lti->id]);
    ...
    public function test_delete_data_for_user() {
        ...
        $contextlist = new \core_privacy\local\request\approved_contextlist($user1, 'lticustom',
        ...
        $count = $DB->count_records('lticustom_submission', ['ltiid' => $lti->id, 'userid' => $user1->id]);
        $this->assertEquals(0, $count);

        // Check the submission for the other user is still there.
        $ltisubmission = $DB->get_records('lticustom_submission');
    ...
    public function test_delete_data_for_users() {
        ...
        $count = $DB->count_records_select('lticustom_submission', $sql, $params);
        $this->assertEquals(0, $count);
    ...
    protected function create_lti_submission(int $ltiid, int $userid) {
        ...
        $DB->insert_record('lticustom_submission', $ltisubmissiondata);
    }
}
