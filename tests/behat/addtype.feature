...
Feature: Add preconfigured tools via teacher interface
  ...
  Scenario: Add a tool from a cartridge
    ...
    And I set the field "Tool URL" to local url "/mod/lticustom/tests/fixtures/ims_cartridge_basic_lti_link.xml"
  ...
  Scenario: Add and use a preconfigured tool
    ...
    And I open "Test tool activity 1" actions menu
    And I choose "Edit settings" in the open action menu
    And I set the field "Tool URL" to local url "/mod/lticustom/tests/fixtures/tool_provider.php"
    And I press "Save and return to course"
