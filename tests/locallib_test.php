 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_lticustom_locallib_testcase extends advanced_testcase {
    ...
    public function test_split_custom_parameters() {
        ...
        $this->assertEquals(lticustom_split_custom_parameters(null, $tool, array(), "x=1\ny=2", false),
            array('custom_x' => '1', 'custom_y' => '2'));

        // Check params with caps.
        $this->assertEquals(lticustom_split_custom_parameters(null, $tool, array(), "X=1", true),
            array('custom_x' => '1', 'custom_X' => '1'));

        // Removed repeat of previous test with a semicolon separator.

        $this->assertEquals(lticustom_split_custom_parameters(null, $tool, array(), 'Review:Chapter=1.2.56', true),
            array(
                'custom_review_chapter' => '1.2.56',
                'custom_Review:Chapter' => '1.2.56'));

        $this->assertEquals(lticustom_split_custom_parameters(null, $tool, array(),
        ...
        $this->assertEquals(array('custom_x' => '1', 'custom_y' => 'SOMETHING'),
            lticustom_split_custom_parameters(null, $tool, array(), "x=1\ny=\$Person.name.middle", false));
    }
    ...
    public function disabled_test_sign_parameters() {
        ...
        $parms = lticustom_sign_parameters($requestparams, 'http://www.imsglobal.org/developer/LTI/tool.php', 'POST',
...
';
        ...
        $parsed = lticustom_parse_grade_replace_message(new SimpleXMLElement($message));
    ...
    public function test_lticustom_ensure_url_is_https() {
        $this->assertEquals('https://moodle.org', lticustom_ensure_url_is_https('http://moodle.org'));
        $this->assertEquals('https://moodle.org', lticustom_ensure_url_is_https('moodle.org'));
        $this->assertEquals('https://moodle.org', lticustom_ensure_url_is_https('https://moodle.org'));
    }

    /**
     * Test lticustom_get_url_thumbprint against various URLs
     */
    public function test_lticustom_get_url_thumbprint() {
        // Note: trailing and double slash are expected right now.  Must evaluate if it must be removed at some point.
        $this->assertEquals('moodle.org/', lticustom_get_url_thumbprint('http://MOODLE.ORG'));
        $this->assertEquals('moodle.org/', lticustom_get_url_thumbprint('http://www.moodle.org'));
        $this->assertEquals('moodle.org/', lticustom_get_url_thumbprint('https://www.moodle.org'));
        $this->assertEquals('moodle.org/', lticustom_get_url_thumbprint('moodle.org'));
        $this->assertEquals('moodle.org//this/is/moodle', lticustom_get_url_thumbprint('http://moodle.org/this/is/moodle'));
        $this->assertEquals('moodle.org//this/is/moodle', lticustom_get_url_thumbprint('https://moodle.org/this/is/moodle'));
        $this->assertEquals('moodle.org//this/is/moodle', lticustom_get_url_thumbprint('moodle.org/this/is/moodle'));
        $this->assertEquals('moodle.org//this/is/moodle', lticustom_get_url_thumbprint('moodle.org/this/is/moodle?'));
        $this->assertEquals('moodle.org//this/is/moodle?foo=bar', lticustom_get_url_thumbprint('moodle.org/this/is/moodle?foo=bar'));
    }

    /*
     * Verify that lticustom_build_request does handle resource_link_id as expected
     */
    public function test_lti_buid_request_resource_link_id() {
        ...
        $instance = $this->getDataGenerator()->create_module('lticustom', array(
            'intro'       => "<p>This</p>\nhas\r\n<p>some</p>\nnew\n\rlines",
        ...
        $params = lticustom_build_request($instance, $typeconfig, $course, null);
    ...
    /**
     * Test lticustom_build_request's resource_link_description and ensure
     * that the newlines in the description are correct.
     */
    public function test_lticustom_build_request_description() {
        $this->resetAfterTest();
    ...
    /**
     * Tests lticustom_prepare_type_for_save's handling of the "Force SSL" configuration.
     */
    public function test_lticustom_prepare_type_for_save_forcessl() {
        $type = new stdClass();
        $config = new stdClass();
    ...
    /**
     * Tests lticustom_load_type_from_cartridge and lticustom_load_type_if_cartridge
     */
    public function test_lticustom_load_type_from_cartridge() {
        $type = new stdClass();
        $type->lti_toolurl = $this->getExternalTestFileUrl('/ims_cartridge_basic_lti_link.xml');

        lticustom_load_type_if_cartridge($type);
    ...
    /**
     * Tests lticustom_load_tool_from_cartridge and lticustom_load_tool_if_cartridge
     */
    public function test_lticustom_load_tool_from_cartridge() {
        $lti = new stdClass();
        $lti->toolurl = $this->getExternalTestFileUrl('/ims_cartridge_basic_lti_link.xml');

        lticustom_load_tool_if_cartridge($lti);
    ...
    /**
     * Tests for lticustom_build_content_item_selection_request().
     */
    public function test_lticustom_build_content_item_selection_request() {
        $this->resetAfterTest();

        $this->setAdminUser();
        // Create a tool proxy.
        $proxy = mod_lticustom_external::create_tool_proxy('Test proxy', $this->getExternalTestFileUrl('/test.html'), array(), array());

        // Create a tool type, associated with that proxy.
        $type = new stdClass();
        $data = new stdClass();
        $data->lti_contentitem = true;
        $type->state = LTI_CUSTOM_TOOL_STATE_CONFIGURED;
        $type->name = "Test tool";
        $type->description = "Example description";
        $type->toolproxyid = $proxy->id;
        $type->baseurl = $this->getExternalTestFileUrl('/test.html');

        $typeid = lticustom_add_type($type, $data);
        ...
        // Default parameters.
        $result = lticustom_build_content_item_selection_request($typeid, $course, $returnurl);
        ...
        $this->assertEquals(LTI_CUSTOM_VERSION_1, $params['lti_version']);
        ...
        $result = lticustom_build_content_item_selection_request($typeid, $course, $returnurl, $title, $text, $mediatypes, $targets,
    ...
    /**
     * Test for lticustom_build_content_item_selection_request() with nonexistent tool type ID parameter.
     */
    public function test_lticustom_build_content_item_selection_request_invalid_tooltype() {
        $this->resetAfterTest();
        ...
        lticustom_build_content_item_selection_request(1, $course, $returnurl);
    }

    /**
     * Test for lticustom_build_content_item_selection_request() with invalid media types parameter.
     */
    public function test_lticustom_build_content_item_selection_request_invalid_mediatypes() {
        $this->resetAfterTest();
        ...
        lticustom_build_content_item_selection_request($typeid, $course, $returnurl, '', '', $mediatypes);
    }

    /**
     * Test for lticustom_build_content_item_selection_request() with invalid presentation targets parameter.
     */
    public function test_lticustom_build_content_item_selection_request_invalid_presentationtargets() {
        $this->resetAfterTest();
        ...
        $this->expectException('coding_exception');
        lticustom_build_content_item_selection_request($typeid, $course, $returnurl, '', '', [], $targets);
    }

    /**
     * Provider for test_lticustom_get_best_tool_by_url.
     *
     * @return array of [urlToTest, expectedTool, allTools]
     */
    public function lticustom_get_best_tool_by_url_provider() {
        $tools = [
            (object) [
                'name' => 'Here',
                'baseurl' => 'https://example.com/i/am/?where=here',
                'tooldomain' => 'example.com',
                'state' => LTI_CUSTOM_TOOL_STATE_CONFIGURED,
                'course' => SITEID
            ],
            (object) [
                'name' => 'There',
    ...
    /**
     * Test lticustom_get_best_tool_by_url.
     *
     * @dataProvider lticustom_get_best_tool_by_url_provider
     * @param string $url The URL to test.
     * @param object $expected The expected tool matching the URL.
     * @param array $tools The pool of tools to match the URL with.
     */
    public function test_lticustom_get_best_tool_by_url($url, $expected, $tools) {
        $actual = lticustom_get_best_tool_by_url($url, $tools, null);
        $this->assertSame($expected, $actual);
    }

    /**
     * Test lticustom_get_jwt_message_type_mapping().
     */
    public function test_lticustom_get_jwt_message_type_mapping() {
        $mapping = [
        ...
        $this->assertEquals($mapping, lticustom_get_jwt_message_type_mapping());
    }

    /**
     * Test lticustom_get_jwt_claim_mapping()
     */
    public function test_lticustom_get_jwt_claim_mapping() {
        $mapping = [
            'accept_copy_advice' => [
                'suffix' => 'dl',
        ...
        $this->assertEquals($mapping, lticustom_get_jwt_claim_mapping());
    }

    /**
     * Test lticustom_build_standard_message().
     */
    public function test_lticustom_build_standard_message_institution_name_set() {
        global $CFG;

        $this->resetAfterTest();

        $CFG->mod_lticustom_institution_name = 'some institution name lols';

        $course   = $this->getDataGenerator()->create_course();
        $instance = $this->getDataGenerator()->create_module('lticustom',
            [
                'course' => $course->id,
            ]
        );

        $message = lticustom_build_standard_message($instance, '2', LTI_CUSTOM_VERSION_1);

        $this->assertEquals('moodle-2', $message['ext_lms']);
        $this->assertEquals('moodle', $message['tool_consumer_info_product_family_code']);
        $this->assertEquals(LTI_CUSTOM_VERSION_1, $message['lti_version']);
     ...
     * Test lticustom_build_standard_message().
     */
    public function test_lticustom_build_standard_message_institution_name_not_set() {
        $this->resetAfterTest();
        ...
        $message = lticustom_build_standard_message($instance, '2', LTI_CUSTOM_VERSION_2);

        $this->assertEquals('moodle-2', $message['ext_lms']);
        $this->assertEquals('moodle', $message['tool_consumer_info_product_family_code']);
        $this->assertEquals(LTI_CUSTOM_VERSION_2, $message['lti_version']);
        $this->assertEquals('basic-lti-launch-request', $message['lti_message_type']);
        $this->assertEquals('2', $message['tool_consumer_instance_guid']);
    ...
    /**
     * Test lticustom_verify_jwt_signature().
     */
    public function test_lticustom_verify_jwt_signature() {
        $this->resetAfterTest();
...
-----END PUBLIC KEY-----';
        ...
        $typeid = lticustom_add_type($type, $config);

        lticustom_verify_jwt_signature($typeid, '', 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4g' .
     ...
     * Test lticustom_verify_jwt_signature().
     */
    public function test_lticustom_verify_jwt_signature_with_lti2() {
        $this->resetAfterTest();
        ...
        lticustom_verify_jwt_signature($typeid, '', '');
    }

    /**
     * Test lticustom_verify_jwt_signature().
     */
    public function test_lticustom_verify_jwt_signature_no_consumer_key() {
        $this->resetAfterTest();
        ...
        $this->expectExceptionMessage(get_string('errorincorrectconsumerkey', 'mod_lticustom'));
        lticustom_verify_jwt_signature($typeid, '', '');
    }
     ...
     * Test lticustom_verify_jwt_signature().
     */
    public function test_lticustom_verify_jwt_signature_no_public_key() {
        $this->resetAfterTest();
        ...
        lticustom_verify_jwt_signature($typeid, 'consumerkey', '');
    }

    /**
     * Test lticustom_convert_content_items().
     */
    public function test_lticustom_convert_content_items() {
        $contentitems = [];
        $contentitems[] = [
        ...
        $json = lticustom_convert_content_items($contentitems);
    ...
    /**
     * Test lticustom_sign_jwt().
     */
    public function test_lticustom_sign_jwt() {
        $this->resetAfterTest();
        ...
        $jwt = lticustom_sign_jwt($params, $endpoint, $oauthconsumerkey, $typeid, $nonce);
    ...
    /**
     * Test lticustom_convert_from_jwt()
     */
    public function test_lticustom_convert_from_jwt() {
        $this->resetAfterTest();
...
-----END PUBLIC KEY-----';
        ...
        $params = lticustom_convert_from_jwt($typeid, 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwib' .
    ...
    /**
     * Test lticustom_get_permitted_service_scopes().
     */
    public function test_lticustom_get_permitted_service_scopes() {
        $this->resetAfterTest();
        ...
        $typeid = lticustom_add_type($type, $typeconfig);

        $tool = lticustom_get_type($typeid);

        $config = lticustom_get_type_config($typeid);
        $permittedscopes = lticustom_get_permitted_service_scopes($tool, $config);
    ...
    /**
     * Test lticustom_get_tool_type_config().
     */
    public function test_lticustom_get_tool_type_config() {
        $this->resetAfterTest();
        ...
        $type = lticustom_get_type($typeid);

        $typeconfig = lticustom_get_tool_type_config($type);
        ...
        $this->assertEquals('https://www.example.com/moodle/mod/lticustom/certs.php', $typeconfig['publickeyseturl']);
        $this->assertEquals('https://www.example.com/moodle/mod/lticustom/token.php', $typeconfig['accesstokenurl']);
        $this->assertEquals('https://www.example.com/moodle/mod/lticustom/auth.php', $typeconfig['authrequesturl']);
    }

    /**
     * Test lticustom_new_access_token().
     */
    public function test_lticustom_new_access_token() {
        global $DB;
        ...
        lticustom_new_access_token($typeid, $scopes);

        $token = $DB->get_records('lticustom_access_tokens');
        ...
        $this->assertEquals($token->timecreated + LTI_CUSTOM_ACCESS_TOKEN_LIFE, $token->validuntil);
