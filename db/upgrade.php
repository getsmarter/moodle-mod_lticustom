 ...
 * @return boolean
 */
function xmldb_lticustom_upgrade($oldversion) {
    ...
    if ($oldversion < 2017111301) {
        ...
        $ltis = $DB->get_recordset_sql("
            ...
            INNER JOIN {grade_items} gt
             ...
             LEFT JOIN {lticustom_types} t
                    ON t.id = l.typeid
             LEFT JOIN {lticustom_types_config} tc
                    ON tc.typeid = t.id AND tc.name = 'acceptgrades'
                 WHERE gt.itemmodule = 'lticustom'
                   AND gt.itemtype = 'mod'
        ");
        ...
        foreach ($ltis as $lti) {
            ...
            if (!$acceptgrades) {
                ...
                grade_update('mod/lticustom', $lti->course, 'mod', 'lticustom', $lti->id, 0, null, array('deleted' => 1));
            }
        ...
        upgrade_mod_savepoint(true, 2017111301, 'lticustom');
    }
    ...
    if ($oldversion < 2019031300) {
        ...
        $table = new xmldb_table('lticustom_types');
        ...
        if (!$dbman->field_exists($table, $field)) {
            ...
            $DB->set_field_select('lticustom_types', 'ltiversion', 'LTI-1p0', 'toolproxyid IS NULL');
            $DB->set_field_select('lticustom_types', 'ltiversion', 'LTI-2p0', 'toolproxyid IS NOT NULL');
        }
        ...
        upgrade_mod_savepoint(true, 2019031300, 'lticustom');
    }

    if ($oldversion < 2019031301) {
        // Define table lti_access_tokens to be created.
        $table = new xmldb_table('lticustom_access_tokens');
        ...
        $table->add_key('typeid', XMLDB_KEY_FOREIGN, array('typeid'), 'lticustom_types', array('id'));
        ...
        // Lti savepoint reached.
        upgrade_mod_savepoint(true, 2019031301, 'lticustom');
    }

    if ($oldversion < 2019031302) {
        // Define field typeid to be added to lti_tool_settings.
        $table = new xmldb_table('lticustom_tool_settings');
        ...
        $table = new xmldb_table('lticustom_tool_settings');
        $key = new xmldb_key('typeid', XMLDB_KEY_FOREIGN, ['typeid'], 'lticustom_types', ['id']);

        // Launch add key typeid.
        $dbman->add_key($table, $key);

        // Lti savepoint reached.
        upgrade_mod_savepoint(true, 2019031302, 'lticustom');
    }
