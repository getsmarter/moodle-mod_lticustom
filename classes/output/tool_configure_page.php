...
class tool_configure_page implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $keyhelp = new help_icon('resourcekey', 'mod_lticustom');
        $secrethelp = new help_icon('password', 'mod_lticustom');

        $url = new moodle_url('/mod/lticustom/typessettings.php', array('sesskey' => sesskey(), 'returnto' => 'toolconfigure'));
        $data->configuremanualurl = $url->out();
        $url = new moodle_url('/admin/settings.php?section=modsettinglticustom');
        $data->managetoolsurl = $url->out();
        $url = new moodle_url('/mod/lticustom/toolproxies.php');
