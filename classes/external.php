 ...
 * @since      Moodle 3.0
 */
class mod_lticustom_external extends external_api {
    ...
    public static function get_tool_proxies($orphanedonly) {
        ...
        $proxies = lticustom_get_tool_proxies($orphanedonly);

        return array_map('lticustom_serialise_tool_proxy', $proxies);
    }
    ...
    public static function get_tool_launch_data($toolid) {
        ...
        $lti = $DB->get_record('lticustom', array('id' => $params['toolid']), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($lti, 'lticustom');
        ...
        $lti->cmid = $cm->id;
        list($endpoint, $parms) = lticustom_get_launch_data($lti);
    ...
    public static function get_ltis_by_courses($courseids = array()) {
        ...
        if (!empty($params['courseids'])) {
            ...
            foreach ($ltis as $lti) {
                ...
                $viewablefields = [];
                if (has_capability('mod/lticustom:view', $context)) {
                    $options = array('noclean' => true);
                    list($module['intro'], $module['introformat']) =
                        external_format_text($lti->intro, $lti->introformat, $context->id, 'mod_lticustom', 'intro', null, $options);

                    $module['introfiles'] = external_util::get_area_files($context->id, 'mod_lticustom', 'intro', false, false);
    ...
    public static function view_lti($ltiid) {
        ...
        // Request and permission validation.
        $lti = $DB->get_record('lticustom', array('id' => $params['ltiid']), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($lti, 'lticustom');
    ...
    public static function create_tool_proxy($name, $registrationurl, $capabilityoffered, $serviceoffered) {
        ...
        $duplicates = lticustom_get_tool_proxies_from_registration_url($registrationurl);
        if (!empty($duplicates)) {
            throw new moodle_exception('duplicateregurl', 'mod_lticustom');
        }
        ...
        $id = lticustom_add_tool_proxy($config);
    ...
    public static function get_tool_proxy_registration_request($id) {
        ...
        $toolproxy = lticustom_get_tool_proxy($id);
        return lticustom_build_registration_request($toolproxy);
    }
    ...
    public static function get_tool_types($toolproxyid) {
        ...
        if (!empty($toolproxyid)) {
            $types = lticustom_get_lti_types_from_proxy_id($toolproxyid);
        } else {
            $types = lticustom_get_lti_types();
        }

        return array_map("lticustom_serialise_tool_type", array_values($types));
    }
    ...
    public static function create_tool_type($cartridgeurl, $key, $secret) {
        ...
        if (!empty($cartridgeurl)) {
            ...
            $data->lti_sendname = LTI_CUSTOM_SETTING_DELEGATE;
            $data->lti_sendemailaddr = LTI_CUSTOM_SETTING_DELEGATE;
            $data->lti_acceptgrades = LTI_CUSTOM_SETTING_DELEGATE;
            $data->lti_forcessl = 0;
            ...
            lticustom_load_type_from_cartridge($cartridgeurl, $data);
            if (empty($data->lti_toolurl)) {
                throw new moodle_exception('unabletocreatetooltype', 'mod_lticustom');
            } else {
                $id = lticustom_add_type($type, $data);
            }
        }

        if (!empty($id)) {
            $type = lticustom_get_type($id);
            return lticustom_serialise_tool_type($type);
        } else {
            throw new moodle_exception('unabletocreatetooltype', 'mod_lticustom');
        }
    }
    ...
    public static function update_tool_type($id, $name, $description, $state) {
        ...
        if (empty($type)) {
            throw new moodle_exception('unabletofindtooltype', 'mod_lticustom', '', array('id' => $id));
        }
        ...
        lticustom_update_type($type, new stdClass());
    ...
    public static function delete_tool_type($id) {
        ...
        if (!empty($type)) {
            lticustom_delete_type($id);

            // If this is the last type for this proxy then remove the proxy
            // as well so that it isn't orphaned.
            $types = lticustom_get_lti_types_from_proxy_id($type->toolproxyid);
            if (empty($types)) {
                lticustom_delete_tool_proxy($type->toolproxyid);
            }
        }
    ...
    public static function is_cartridge($url) {
        ...
        $iscartridge = lticustom_is_cartridge($url);
