...
namespace mod_lticustom\local\ltiservice;
...
/**
 * The mod_lticustom\local\ltiservice\service_base class.
 *
 * @package    mod_lticustom
 * @since      Moodle 2.8
 * @copyright  2014 Vital Source Technologies http://vitalsource.com
...
abstract class service_base {
    ...
    public function get_component_id() {
        ...
        return 'lticustomservice_' . $this->id;
    ...
    /**
     * Default implementation will check for the existence of at least one mod_lticustom entry for that tool and context.
     *
    ...
    public function is_used_in_context($typeid, $courseid) {
        ...
        $ok = $DB->record_exists('lticustom', array('course' => $courseid, 'typeid' => $typeid));
        return $ok || $DB->record_exists('lticustom_types', array('course' => $courseid, 'id' => $typeid));
    }
    ...
    public function is_allowed_in_context($typeid, $courseid) {
        ...
        $type = $DB->get_record('lticustom_types', array('id' => $typeid));
    ...
    public static function get_service_path() {
        ...
        $url = new \moodle_url('/mod/lticustom/services.php');
    ...
    public function check_tool($typeid, $body = null, $scopes = null) {
        ...
        if ($consumerkey === false) {
            ...
            if (!empty($typeid)) {
                $this->type = lticustom_get_type($typeid);
                $this->typeconfig = lticustom_get_type_config($typeid);
                $ok = !empty($this->type->id);
                if ($ok && !empty($this->type->toolproxyid)) {
                    $this->toolproxy = lticustom_get_tool_proxy($this->type->toolproxyid);
                }
            } else {
    ...
    public function check_tool_proxy($toolproxyguid, $body = null) {
        ...
        if (!empty($toolproxyguid)) {
            $toolproxy = lticustom_get_tool_proxy_from_guid($toolproxyguid);
            if ($toolproxy !== false) {
