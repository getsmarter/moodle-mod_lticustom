...
/**
 * Privacy Subsystem implementation for mod_lticustom.
 *
 * @package    mod_lticustom
 * @copyright  2018 Mark Nelson <markn@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
namespace mod_lticustom\privacy;
...
class provider implements
    ...
    public static function get_metadata(collection $items) : collection {
        ...
        $items->add_database_table(
            'lticustom_submission',
            [
                'userid' => 'privacy:metadata:lticustom_submission:userid',
                'datesubmitted' => 'privacy:metadata:lticustom_submission:datesubmitted',
                'dateupdated' => 'privacy:metadata:lticustom_submission:dateupdated',
                'gradepercent' => 'privacy:metadata:lticustom_submission:gradepercent',
                'originalgrade' => 'privacy:metadata:lticustom_submission:originalgrade',
            ],
            'privacy:metadata:lti_submission'
        );

        $items->add_database_table(
            'lticustom_tool_proxies',
            [
                'name' => 'privacy:metadata:lticustom_tool_proxies:name',
                'createdby' => 'privacy:metadata:createdby',
                'timecreated' => 'privacy:metadata:timecreated',
                'timemodified' => 'privacy:metadata:timemodified'
            ],
            'privacy:metadata:lticustom_tool_proxies'
        );

        $items->add_database_table(
            'lticustom_types',
            [
                'name' => 'privacy:metadata:lticustom_types:name',
                'createdby' => 'privacy:metadata:createdby',
                'timecreated' => 'privacy:metadata:timecreated',
                'timemodified' => 'privacy:metadata:timemodified'
            ],
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        ...
        $sql = "SELECT c.id
            ...
            INNER JOIN {modules} m
                    ON m.id = cm.module
                   AND m.name = :modname
            INNER JOIN {lticustom} lti
                    ON lti.id = cm.instance
            INNER JOIN {lticustom_submission} ltisub
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        self::export_user_data_lticustom_tool_proxies($contextlist);
    }
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        if ($cm = get_coursemodule_from_id('lticustom', $context->instanceid)) {
            $DB->delete_records('lticustom_submission', ['ltiid' => $cm->instance]);
        }
    }
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            $DB->delete_records('lticustom_submission', ['ltiid' => $instanceid, 'userid' => $userid]);
        }
    }
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        if ($context instanceof \context_module) {
            ...
            $DB->delete_records_select('lticustom_submission', $sql, $params);
        }
    }
    ...
    protected static function export_user_data_lti_submissions(approved_contextlist $contextlist) {
        ...
        $recordset = $DB->get_recordset_select('lticustom_submission', "ltiid $insql AND userid = :userid", $params, 'dateupdated, id');
    ...
    protected static function export_user_data_lti_types(approved_contextlist $contextlist) {
        ...
        $params = array_merge($inparams, ['userid' => $user->id]);
        $ltitypes = $DB->get_recordset_select('lticustom_types', "course $insql AND createdby = :userid", $params, 'timecreated ASC');
        self::recordset_loop_and_export($ltitypes, 'course', [], function($carry, $record) {
            ...
            $finaldata = (object) ['lticustom_types' => $data];
            writer::with_context($context)->export_data([], $finaldata);
        });
    }
     ...
     * @param approved_contextlist $contextlist a list of contexts approved for export.
     */
    protected static function export_user_data_lticustom_tool_proxies(approved_contextlist $contextlist) {
        global $DB;
        ...
        $data = [];
        $ltiproxies = $DB->get_recordset('lticustom_tool_proxies', ['createdby' => $user->id], 'timecreated ASC');
        ...
        $finaldata = (object) ['lticustom_tool_proxies' => $data];
    ...
    protected static function get_lti_ids_to_cmids_from_cmids(array $cmids) {
        ...
        $sql = "SELECT lti.id, cm.id AS cmid
                 ...
                 JOIN {course_modules} cm
                  ...
                  AND cm.module = m.id
                WHERE cm.id $insql";
        $params = array_merge($inparams, ['lti' => 'lticustom']);
