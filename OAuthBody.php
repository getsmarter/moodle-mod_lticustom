...
namespace moodle\mod\lticustom; // Using a namespace as the basicLTI module imports classes with the same names.
...
function get_oauth_key_from_headers($typeid = null, $scopes = null) {
    ...
    if (isset($requestheaders['Authorization'])) {
        if (substr($requestheaders['Authorization'], 0, 6) == "OAuth ") {
            ...
            $token = $DB->get_record('lticustom_access_tokens', $conditions);
            if ($token) {
                // Log token access.
                $DB->set_field('lticustom_access_tokens', 'lastaccess', $now, array('id' => $token->id));
