...
if ($ok) {
    ...
    $tool = $DB->get_record('lticustom_types', array('clientid' => $claims['sub']));
    if ($tool) {
        $typeconfig = lticustom_get_type_config($tool->id);
...
if ($ok) {
    ...
    $permittedscopes = lticustom_get_permitted_service_scopes($tool, $typeconfig);
...
if ($ok) {
    $token = lticustom_new_access_token($tool->id, $scopes);
    $expiry = LTI_CUSTOM_ACCESS_TOKEN_LIFE;
